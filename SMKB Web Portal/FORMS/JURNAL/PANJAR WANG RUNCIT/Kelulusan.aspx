<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/NestedMasterPage2.master" CodeBehind="Kelulusan.aspx.vb" Inherits="SMKB_Web_Portal.Kelulusan" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="FormContents" runat="server">
    ...
    <div id="KelulusanTab" class="tabcontent" style="display: block">
                                        ...
                                        <table class="table table-striped" id="tblData">
                                            ...
                                            <tbody id="tableID">
                                                <tr style="display: none; width: 100%" class="table-list">
                                                    <td style="width: 20%">
                                                        <select class="ui search dropdown COA-list" name="ddlCOA" id="ddlCOA"
                                                            style="width: 100%">
                                                        </select>
                                                    ...
                                                    <td>
                                                        <label id="lblButir" name="lblButir" class="label-butir-list"></label>
                                                        <label id="HidlblButir" name="HidlblButir" class="Hid-butir-list"
                                                            style="visibility: hidden">
                                                        </label>
                                                    </td>
                                                    <td style="width: 10%">
                                                        <input id="Jumlah" name="Jumlah" runat="server" type="number"
                                                            class="form-control underline-input Jumlah" style="text-align: right" readonly />
                                                    </td>
                                                    <td style="width: 10%">
                                                        <input id="Baki" name="Baki" runat="server" type="number"
                                                            class="form-control underline-input Baki" style="text-align: right" readonly />
                                                    </td>
                                                </tr>
                                            </tbody>
                                            <tfoot>
        ...
        <script>
            ...
            $(document).ready(function () {
                ...
                tbl = $("#tblDataSenarai_perm").DataTable({
                    ...
                    ajax: {
                        url: "Panjar_WS.asmx/LoadOrderRecord_SenaraiLulusPermohonan",
                        method: 'POST',
            ...
            $('.btnLulus').off('click').on('click', async function () {
                ...
                var ptj = $('#lblPtj').text();
                var kump = $('#ddlKumpulan').val();
                var baki = parseFloat($('#lblBakiSemasa').val()).toFixed(2);
                ...
                $('#confirmButton').off('click').on('click', async function () {
                    ...
                    var result = JSON.parse(await ajaxSaveOrderLulus(id, ptj, kump, baki));
            ...
            $('.btnXLulus').off('click').on('click', async function () {
                ...
                msg = "Anda pasti untuk TIDAK meluluskan permohonan ini";
                ...
                $('#confirmButton').off('click').on('click', async function () {
                    ...
                    var result = JSON.parse(await AjaxSaveOrderXLulus(id));
            ...
            async function initDropdownCOA(id) {
                $('#' + id).dropdown({
                    ...
                    onChange: function (value, text, $selectedItem) {
                        ...
                        var recordIDButirHd = curTR.find("td > .label-ptj-list");
                        recordIDButirHd.html($($selectedItem).data("coltambah1"));
                ...
                while (counter <= totalClone) {
                    ...
                    if (objOrder !== null && objOrder !== undefined) {
                        ...
                        if (counter <= objOrder.Payload.length) {
                            await setValueToRow_Kelulusan(row, objOrder.Payload[counter - 1]);
                        }
                    }
                    counter += 1;
                }
            ...
            async function setValueToRow_HdrKelulusan(orderMDetail) {
            ...
            $(tableID).on('keyup', '.Mohon, .Jumlah , .Baki', async function () {

                var curTR = $(this).closest("tr");

                var jumlah_ = $(curTR).find("td > .Mohon");
                var totalDebit = NumDefault(jumlah_.val())

                var baki_ = $(curTR).find("td > .Baki");
                calculateGrandTotal();

                var totalBaki = NumDefault(baki_.val())
                calculateGrandTotal();

            });

            async function clearNum() {
                $('#lblBakiSemasa').val("");
                $('#lblJumlah').val("");
            ...
            async function calculateGrandTotal() {
                ...
                //Jumlah Permohonan
                var totalJumlahMohon = $('#lblJumPermohonan');
                var curTotal_Mohon = 0;

                $('.Mohon').each(function (index, obj) {
                    curTotal_Mohon += parseFloat(NumDefault($(obj).val()));
                });
                totalJumlahMohon.val(curTotal_Mohon.toFixed(2));
            }
            ...
            async function setValueToRow_Kelulusan(row, orderMDetail) {
                ...
                var butir = $(row).find("td > .label-butir-list");
            ...
            async function AjaxLoadOrderRecord_Senarai() {

                try {
                    const response = await fetch('Panjar_WS.asmx/LoadOrderRecord_SenaraiLulusPermohonan', {
                        method: 'POST',
                        headers: {
            ...
            async function AddRowHeader(totalClone, objOrder) {
                ...
                if (counter <= objOrder.Payload.length) {
                    await setValueToRow_HdrKelulusan(objOrder.Payload[counter - 1]);
                }
            ...
            async function ajaxSaveOrderLulus(id, ptj, kump, baki) {

                return new Promise((resolve, reject) => {
                    $.ajax({
                        url: 'Panjar_WS.asmx/LulusOrder',
                        method: 'POST',
                        data: JSON.stringify({ id: id, ptj: ptj, kump: kump, baki: baki }),
                        dataType: 'json',
                        contentType: 'application/json; charset=utf-8',
                        success: function (data) {
                            resolve(data.d);
            ...
            async function AjaxSaveOrderXLulus(id) {

                return new Promise((resolve, reject) => {
                    $.ajax({
                        url: 'Panjar_WS.asmx/XLulusOrder',
                        method: 'POST',
