<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/NestedMasterPage2.master" CodeBehind="UntungRugiDetail.aspx.vb" Inherits="SMKB_Web_Portal.UntungRugiDetail" %>
<%@ Register Src="~/FORMS/JURNAL/LAPORAN/PrintReport/TableVotDetail.ascx" TagPrefix="Vot" TagName="Table" %>
...
<%--    <script type="text/javascript" src="<%=ResolveClientUrl("~/Scripts/select2custom.min.js")%>"></script>--%>
    ...
    <div id="PermohonanTab" class="tabcontent" style="display: block">
            ...
            <div>
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalCenterTitle">Laporan Untung Rugi Terperinci</h5>
        ...
        <script type="text/javascript">
            ...
            var prevIdChild = null;
            var prevRowChild = null;
            var prevTrChild = null;
            var prevBtnChild = null;
            var childTable = null;

            var thirdChildtable = null;
...
<%--                                window.location.href = '<%=ResolveClientUrl("~/reportbaru4.aspx")%>';--%>
                                window.open('<%=ResolveClientUrl("~/FORMS/JURNAL/LAPORAN/PrintReport/reportpnldetail.aspx")%>', '_blank');
                    ...
                    "ajax":
                    {
                        "url": "LejarPenghutangWS.asmx/LoadOrderRecord_PNLDetail",
                        type: 'POST',
                        data: function (d) {
                            return "{ bulan: '" + $('#<%=ddlMonths.ClientID%>').val() + "', tahun: '" + $('#<%=ddlyear.ClientID%>').val() + "',syarikat: '" + $('#<%=DropDownList2.ClientID%>').val() + "', ptj: '" + $('#<%=DropDownList1.ClientID%>').val() + "', kodvot: '', 'level': '1'}"
                        },
                    ...
                    "columns": [
                        {
                            ...
                            render: function (data) {
                                return '<button class="btnDetails" data-level= "2" data-kodvot = "' + data + '"><i class="fas fa-plus"></i></button> ' + data
                            }
                        },
                ...
                function format(rowData) {
                    var childTable = '<table id="cl' + rowData.kodvot + '" class="display compact nowrap w-100" width="100%">' +
                        '<thead style="display:none"></thead >' +
                        '</table>';
                    return $(childTable).toArray();
                }
               
                $('#tblDataSenarai_trans_rpt').on('click', '.btnDetails', function (evt) {
                    evt.preventDefault();
                    var pickedKodvot = $(this).data("kodvot");
                    ...
                    if (row.child.isShown()) {
                        ...
                        $('#cl' + rowData.kodvot).DataTable().destroy();
                    }
                    else {
                        ...
                        prevBtn = $(this);
                        prevTr = tr;
                        prevId = '#cl' + rowData.kodvot
                        prevRow = row;
                        
                        $(this).html('<i class="fas fa-minus"></i>');
                        // Open this row
                        row.child(format(rowData)).show();
                        var id = rowData.kodvot;

                        childTable = $('#cl' + rowData.kodvot).DataTable({
                            dom: "t",
                            ajax: {
                                url: "LejarPenghutangWS.asmx/LoadOrderRecord_PNLDetail",
                                type: 'POST',
                                data: function (d) {
                                    return "{ bulan: '" + $('#<%=ddlMonths.ClientID%>').val() + "', tahun: '" + $('#<%=ddlyear.ClientID%>').val() + "',syarikat: '" + $('#<%=DropDownList2.ClientID%>').val() + "', ptj: '" + $('#<%=DropDownList1.ClientID%>').val() + "', kodvot: '" + pickedKodvot + "', 'level' : '" + level + "'}"                                    
                                },
                                "contentType": "application/json; charset=utf-8",
                                "dataType": "json",
                                "dataSrc": function (json) {
                            ...
                            columns: [
                                ...
                                //},
                                {
                                    ...
                                    "width": "20%",
                                    render: function (data) {
                                        return '<button class="btnDetails2" data-level= "3" data-kodvot = "' + data + '"><i class="fas fa-plus"></i></button> ' + data
                                    }
                                },
                                {
                                    data: "ButiranDetail"
                            ...
                            columnDefs: [
                                ...
                                {
                                    targets: [2, 3],
                                    className: "fee-col2 align-right"
                                }
                            ],
                            select: false,
                        });
                ...
                $('tbody').on('click', '.btnDetails2', function (evt) {
                    evt.preventDefault();
                    var pickedKodvot = $(this).data("kodvot");
                    ...
                    if (row.child.isShown()) {
                        ...
                        $(this).html('<i class="fas fa-plus"></i>');
                        // Destroy the Child Datatable
                        console.log(rowData.kodvot);
                        $('#cl' + rowData.kodvot).DataTable().destroy();
                    }
                    else {

                        if (prevRowChild !== null) {
                            // This row is already open - close it
                            prevRowChild.child.hide();
                            prevTrChild.removeClass('shown');
                            prevBtnChild.html('<i class="fas fa-plus"></i>');
                            // Destroy the Child Datatable
                            $(prevIdChild).DataTable().destroy();
                        }
                        ...
                        thirdChildtable = $('#cl' + rowData.kodvot).DataTable({
                            dom: "t",
                            ajax: {
                                url: "LejarPenghutangWS.asmx/LoadOrderRecord_PNLDetail",
                                type: 'POST',
