...
Public Class KW
    ...
    Private Function fCreateDtKW() As DataTable
        ...
        Try
            ...
            Dim dt As New DataTable
            dt.Columns.Add("Kod_Kump_Wang", GetType(String))
            ...
            Dim strSql As String = "select Kod_Kump_Wang, Butiran, Status from SMKB_Kump_Wang order by Kod_Kump_Wang"
    ...
    Private Sub lbtnSimpan_Click(sender As Object, e As EventArgs) Handles lbtnSimpan.ServerClick
        ...
        Dim dbconn As New DBKewConn
        Try
            Dim strKodKW As String = Trim(txtKodKW.Value.TrimEnd)
            ...
            strSql = "select count(*) from SMKB_Kump_Wang where Kod_Kump_Wang = '" & strKodKW & "'"
            If fCheckRec(strSql) = 0 Then

                strSql = "insert into SMKB_Kump_Wang (Kod_Kump_Wang , Butiran, Status) values(@KodKW,@Butiran,@Status)"
            ...
            Else
                'UPDATE
                strSql = "update SMKB_Kump_Wang set Butiran = @Butiran, Status = @Status where Kod_Kump_Wang = @KodKw"
                Dim paramSql() As SqlParameter = {
                            New SqlParameter("@KodKW", strKodKW),
                            New SqlParameter("@Butiran", strButiran),
                            New SqlParameter("@Status", intStatus)
