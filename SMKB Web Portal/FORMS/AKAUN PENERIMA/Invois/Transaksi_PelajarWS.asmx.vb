...
<ToolboxItem(False)>
Public Class Transaksi_PelajarWS
    ...
    Private Function GetKodUrusniagaList(kod As String) As DataTable
        Dim db = New DBKewConn
        Dim query As String = "SELECT Kod_Detail AS value,Butiran AS text FROM SMKB_Lookup_Detail WHERE Kod='AR04' AND Keutamaan='2' AND Status='1'"
        Dim param As New List(Of SqlParameter)
        If kod <> "" Then
    ...
    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function SaveOrders(order As BilPelajar) As String
        Dim resp As New ResponseRepository
        ...
        For Each orderDetail As BilPelajarDetail In order.OrderDetails
    ...
    'orer detail sini <>
    Public Function InsertOrderDetail(orderDetail As BilPelajarDetail)
        Dim db = New DBKewConn
    ...
    Public Function UpdateOrderDetail(orderDetail As BilPelajarDetail)
        Dim db = New DBKewConn
        Dim data
    ...
    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function SubmitOrders(order As BilPelajar) As String
        Dim resp As New ResponseRepository
        resp.Success("Data telah disimpan")
        Dim success As Integer = 0
        Dim JumRekod As Integer = 0
    ...
    Private Function GetKategoriPenghutang(kod As String) As DataTable
        ...
        Dim query As String = "SELECT Kod_Detail AS value, Butiran AS text
                                ...
                                WHERE Kod='0152' AND Kod_Detail IN ('PL')"
        Return db.Read(query)
    End Function

    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function GetSesiList(ByVal q As String) As String
        Dim tmpDT As DataTable = GetSesi(q)
        Return JsonConvert.SerializeObject(tmpDT)
    End Function

    Private Function GetSesi(kod As String) As DataTable
        Dim db = New DBSMPConn
        Dim query As String = "SELECT KodSesi_Sem AS value, KodSesi_Sem AS text 
                                FROM SMP_SesiPengajian
                                WHERE Keaktifan='1' AND Stat_Skrg='1'"
        Return db.Read(query)
    End Function

    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function GetfakultiList(ByVal q As String) As String
        Dim tmpDT As DataTable = Getfakulti(q)
        Return JsonConvert.SerializeObject(tmpDT)
    End Function

    Private Function Getfakulti(kod As String) As DataTable
        Dim db = New DBKewConn
        Dim query As String = "SELECT Singkatan AS value, Pejabat AS text
                                FROM VPEJABAT 
    ...
    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function LoadSenaraiPelajar(KatPel As String, Sesi As String, Fakulti As String) As String
        Dim resp As New ResponseRepository
        dt = GetSenaraiPelajar(KatPel, Sesi, Fakulti)
        'resp.SuccessPayload(dt)
    ...
    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Private Function GetSenaraiPelajar(KatPel As String, Sesi As String, Fakulti As String) As DataTable
        Dim dbSMP = New DBSMPConn
        Dim query As String
        Dim param As New List(Of SqlParameter)

        If KatPel = "PL" Then 'UG
            query = " SELECT SMP01_Nama,SMP01_Nomatrik,SMP01_Fakulti,SMP01_Kursus,SMP01_Biasiswa
                    FROM SMP01_Peribadi
                    WHERE SMP01_Status='AKTIF' AND SMP01_Fakulti=@fakulti
                    ORDER BY SMP01_Kursus ASC,SMP01_Nomatrik ASC "

            param.Add(New SqlParameter("@fakulti", Fakulti))

            Return dbSMP.Read(query, param)
        ElseIf KatPel = "PG" Then 'PG
            query = " AND CAST(Tkh_Mohon AS DATE) = CAST(DATEADD(day, -1, CURRENT_TIMESTAMP) AS DATE) "
        ElseIf KatPel = "SH" Then 'SH
            query = " AND Tkh_Mohon >= DATEADD(day, -7, CURRENT_TIMESTAMP) AND Tkh_Mohon < CURRENT_TIMESTAMP "
        End If
