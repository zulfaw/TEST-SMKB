...
Public Class MaklumatSemakanBajet_NC_RV
    ...
    Protected Sub gvButiran_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvButiran.RowCommand
        ...
        If e.CommandName = "Padam" Then
            If noMohon <> "" Then
                ...
                Dim strSQL_ = $"UPDATE 
                                BG01_MOHONDt_RV SET  BG20_FlagNC_Delete = @Status Where BG20_NoMohon = '{noMohon}' and BG20_Id = '{idDt}'"
    ...
    Private Sub MohonEdit(NoMohon As String)
        ...
        Try
            ...
            Dim strSqlDt = $"Select BG20_Id, BG20_NoButiran,BG20_Butiran,BG20_Kuantiti,CASE WHEN BG20_JumNC IS NOT NULL  THEN ISNULL((BG20_JumNC),0) ELSE ISNULL((BG20_JumKetuaPTj),0) END AS BG20_AmaunMohon,BG20_AngJumlah,KodVotSebagai,KodVotAm, BG20_Butiran from BG01_MOHONDt_RV where BG20_NoMohon='{NoMohon}' and BG20_Aktif = '1'"
    ...
    Protected Sub lbtnKembali_Click(sender As Object, e As EventArgs) Handles lbtnKembali.Click
        ...
        If lblKodSkrin.Text = "2" Then
            Response.Redirect($"~/FORMS/BAJET/SEMAKAN BAJET/Maklumat_Agihan_Bajet_Am_NC_RV.aspx?KodSub=" & strKodSub & "&KodSubMenu=" & strKodSubMenu & "&KodPTj=" & strKodPTj & "&Tahun=" & strTahun & "&KodKW=" & strKodKW & "&kodVot=" & strKodVot & "&strKodKO=" & strKodKO & "", False)
        Else
            Response.Redirect("~/FORMS/BAJET/SEMAKAN BAJET/Pengesahan_NC.aspx")
        End If
    ...
    Protected Sub lbtnHapus_Click(sender As Object, e As EventArgs) Handles lbtnHapus.Click
        ...
        If countButiran > 0 Then
            fGlobalAlert($"Rekod telah dipadam!", Me.Page, Me.[GetType](), "../../BAJET/SEMAKAN BAJET/Pengesahan_NC.aspx")
        Else
            ...
            If Page.IsValid Then
                ...
                If txtNoMohon.Text IsNot String.Empty Then
                    ...
                    If dbconn.fUpdateCommand(strSQL_Save, paramSql_simpan) > 0 Then
                        ...
                        Dim strSQL1_ = $"UPDATE 
                                BG01_MOHONDt_RV SET  BG20_Aktif = @Status Where BG20_NoMohon = '{noPermohonan}' and BG20_FlagNC_Delete = '0' "
                        ...
                        For Each drCurrentRow As DataRow In dtCurrentTable.Rows
                            ...
                            If IsDBNull(drCurrentRow("ID")) Then
                                ...
                                strSqlButiran = "INSERT INTO BG01_MOHONDt_RV ( BG20_NoButiran, BG20_NoMohon, KodVotAm, KodVotSebagai,   BG20_JumNC, BG20_AngJumlah,  BG20_Butiran, BG20_Aktif)
                        ...
                        If countButiran > 0 Then
                            If lblKodSkrin.Text = "2" Then
                                ...
                                fGlobalAlert($"Rekod baru telah ditambah bersama '{countButiran.ToString}' butiran permohonan!", Me.Page, Me.[GetType](), $"../../BAJET/SEMAKAN BAJET/MaklumatSemakanBajet_NC_RV.aspx?KodSub={KodSub}&KodSubMenu={KodSubMenu}&kodSkrin=2&no={strNoMohon}&kodvot={kodVot}")

                            Else

                                fGlobalAlert($"Rekod baru telah ditambah bersama '{countButiran.ToString}' butiran permohonan!", Me.Page, Me.[GetType](), "../../BAJET/SEMAKAN BAJET/Pengesahan_NC.aspx")
                ...
                Else
                    ...
                    Dim paramSql2() As SqlParameter =
                  {
                    ...
                    If dbconn.fInsertCommand(strSql, paramSql) > 0 Then
                        ...
                        If countButiran > 0 Then
                            fGlobalAlert($"Rekod telah dikemaskini bersama '{countButiran.ToString}' butiran permohonan!", Me.Page, Me.[GetType](), "../../BAJET/SEMAKAN BAJET/Pengesahan_NC.aspx")
                        Else
                            fGlobalAlert("Maklumat Butiran tidak berjaya disimpan!", Me.Page, Me.[GetType]())
                        End If
