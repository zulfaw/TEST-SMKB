...
AND KodBahagian = '4101')
    ...
    Private Function fGetAmtVotAm(strTahun, strKodKW, strKO, strKodPTj, strVotAm) As Decimal
        Try
            ...
            strSql = $"select case when ISNULL(SUM(bjam.JumlahNC ),0) = 0 then
ISNULL(SUM(BG20_JumNC),0) 
else
ISNULL(SUM(bjam.JumlahNC ),0)
end as BG06_Amaun from BG01_Mohon_RV as a inner join BG01_MohonDt_RV as b on a.BG20_NoMohon = b.BG20_NoMohon left join BG01_JumlahBajetAm as bjam 
...
where a.BG20_TahunBajet = '{strTahun}' and a.KodKW = '{strKodKW}' and a.KodOperasi = '{strKO}' and a.KodPtj = '{strKodPTj}' and b.KodVotAm = '{strVotAm}'  and b.BG20_Aktif ='1' and BG20_FlagBend  = '1'"
...
where MK01_Tahun = '" & Date.Now.Year.ToString & "' and MK01_VotTahun.KodKw = '" & strKodKW & "' order by MK_PTJ .KodPTJ"
    ...
    Private Sub fBindGVMohonBajet(KodStatus As String)
        Try
            ...
            If KodStatus = String.Empty Then
                strSql = $"select DISTINCT a.BG20_NoMohon, BG20_TarikhMohon, BG20_Program, ISNULL((BG20_AmaunBajet),0) as BG20_AmaunMohon, StatusDok, KodKW, KodOperasi, BG20_Justifikasi from BG01_Mohon_RV as a WHERE a.BG20_Status = 1 and BG20_FlagBend = '1' " + ayatSql + " ORDER BY BG20_NoMohon DESC,  BG20_TarikhMohon DESC"
                strSql_count = $"select count(DISTINCT a.BG20_NoMohon) as bil  from BG01_Mohon_RV as a WHERE a.BG20_Status = 1 and BG20_FlagBend = '1'  " + ayatSql
            Else
                strSql = $"select DISTINCT a.BG20_NoMohon, BG20_TarikhMohon, BG20_Program, ISNULL((BG20_AmaunBajet),0) as BG20_AmaunMohon, StatusDok, KodKW, KodOperasi, BG20_Justifikasi from BG01_Mohon_RV as a where StatusDok='{KodStatus}' and BG20_FlagBend = '1' " + ayatSql + " ORDER BY BG20_NoMohon DESC,  BG20_TarikhMohon DESC"
                strSql_count = $"select count(DISTINCT a.BG20_NoMohon) as bil  from BG01_Mohon_RV as a WHERE a.BG20_Status = 1 and BG20_FlagBend = '1'  " + ayatSql
            End If
    ...
    Protected Sub gvAgihPTj_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvAgihPTj.RowCommand
        ...
        If e.CommandName = "Select" Then
            ...
            'Open other page.
            Response.Redirect($"~/FORMS/BAJET/SEMAKAN BAJET/MaklumatSemakanBajet_NC_RV.aspx?KodSubMenu={KodSubMenu}&no={NoMohon}")
    ...
    Protected Sub lbtnHantar_Click(sender As Object, e As EventArgs) Handles lbtnHantar.Click
        ...
        Else
            ...
            If countButiran > 0 Then
                fGlobalAlert($"Berjaya simpan", Me.Page, Me.[GetType](), "../../BAJET/SEMAKAN BAJET/Pengesahan_NC.aspx")
            Else
    ...
    Protected Sub lblKemaskiniVot_Click(sender As Object, e As EventArgs) Handles lblKemaskiniVot.Click
        ...
        If dbconn_.fUpdateCommand(strSQL_Save_, paramSql_simpan_) > 0 Then
            ...
            If countButiran > 0 Then
                fGlobalAlert($"Maklumat berjaya dikemaskini.", Me.Page, Me.[GetType](), "../../BAJET/SEMAKAN BAJET/Pengesahan_NC.aspx")
            Else
                fGlobalAlert("Maklumat Butiran tidak berjaya disimpan!", Me.Page, Me.[GetType]())
            End If
    ...
    Private Sub gvMohonBajet_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gvMohonBajet.SelectedIndexChanged
        ...
        'Open other page.
        Response.Redirect($"~/FORMS/BAJET/SEMAKAN BAJET/MaklumatSemakanBajet_NC_RV.aspx?KodSub={KodSub}&KodSubMenu={KodSubMenu}&kodSkrin=1&no={strNoMohon}", False)
    End Sub
End Class
