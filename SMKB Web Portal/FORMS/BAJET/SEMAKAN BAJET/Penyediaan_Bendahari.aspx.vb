...
Public Class Penyediaan_Bendahari
...
from BG01_MohonDt_RV a where  LEFT(a.KodVotSebagai, 1) <> '0' and KodVotSebagai = '11000' group by a.KodVotSebagai"
    ...
    Private Sub sBindGvProgram(ByVal strKodVot As String)
        Try
            ...
            Dim ds As New DataSet
            Dim strSql As String = "select BG20_NoMohon , KodBahagian  , KodUnitPtj ,BG20_Program,BG20_AmaunMohon from BG01_Mohon_RVwhere BG20_NoMohon in (select BG20_NoMohon  from BG01_MohonDt_RV where KodVotSebagai = '11000'
AND KodBahagian = '4101')
    ...
    '            strSql = "update BG01_Mohon_RV set  StatusDok = @StatusDok where BG20_NoMohon in (select BG20_NoMohon from BG01_Mohon_RVwhere StatusDok = '05' and BG20_Status = 1)"
    '            Dim paramSql() As SqlParameter = {
    ...
    Private Function fGetAmaunMohon() As String
        Dim strSql As String
        Try
            strSql = "Select sum(BG20_AmaunMohon ) As AmaunMohon from BG01_Mohon_RVwhere KodPtj = '" & Session("ssusrKodPTj") & "'"
            Dim ds As New DataSet
            Dim dbconn As New DBKewConn
            Dim strAmaunMohon As String
    ...
    Private Function fFindRec() As DataTable
        Try
            ...
            '                   From BG01_Mohon_RVa INNER Join BG01_MohonDt_RV b On a.BG20_NoMohon = b.BG20_NoMohon
            '                   LEFT OUTER JOIN
    ...
    Private Function fGetAmtVotAm(strTahun, strKodKW, strKO, strKodPTj, strVotAm) As Decimal
        Try
            ...
            strSql = $"select isnull(sum(b.BG20_AngJumlah),0) as BG06_Amaun from BG01_Mohon_RV as a inner join BG01_MohonDt_RV as b on a.BG20_NoMohon = b.BG20_NoMohon where a.BG20_TahunBajet = '{strTahun}' and a.KodKW = '{strKodKW}' and a.KodOperasi = '{strKO}' and a.KodPtj = '{strKodPTj}' and b.KodVotAm = '{strVotAm}' and b.BG20_Aktif ='1' and B20_FlagPTj = '1'"
    ...
    Private Sub ddlKW_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlKW.SelectedIndexChanged
        ...
        Try
            ...
            If ddlKW.SelectedIndex <> 0 Then
                Dim val_Kw = ddlKW.SelectedValue
                fBindGvPTj(val_Kw)
            End If
    ...
    Private Sub fBindGvPTj(strKodKW As String)
        ...
        Try
            ...
            Dim strTahun = dateNext.Year 'ddlTahunAgih.SelectedValue.TrimEnd
    ...
    Private Sub fBindGVMohonBajet(KodStatus As String)
        Try
            ...
            If KodStatus = String.Empty Then
                strSql = $"select DISTINCT a.BG20_NoMohon, BG20_TarikhMohon, BG20_Program, BG20_AmaunMohon as BG20_AmaunMohon, StatusDok, KodKW, KodOperasi, BG20_Justifikasi from BG01_Mohon_RV as a WHERE a.BG20_Status = 1 and B20_FlagPTj = '1' " + ayatSql + " ORDER BY BG20_NoMohon DESC,  BG20_TarikhMohon DESC"
                strSql_count = $"select count(DISTINCT a.BG20_NoMohon) as bil  from BG01_Mohon_RV as a WHERE a.BG20_Status = 1 and B20_FlagPTj = '1'  " + ayatSql
            Else
                strSql = $"select DISTINCT a.BG20_NoMohon, BG20_TarikhMohon, BG20_Program, BG20_AmaunMohon as BG20_AmaunMohon, StatusDok, KodKW, KodOperasi, BG20_Justifikasi from BG01_Mohon_RV as a where StatusDok='{KodStatus}'  and B20_FlagPTj = '1' " + ayatSql + " ORDER BY BG20_NoMohon DESC,  BG20_TarikhMohon DESC"
                strSql_count = $"select count(DISTINCT a.BG20_NoMohon) as bil  from BG01_Mohon_RV as a WHERE a.BG20_Status = 1 and B20_FlagPTj = '1'  " + ayatSql
            End If
    ...
    Protected Sub gvMohonBajet_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvMohonBajet.RowCommand
        ...
        If e.CommandName = "Select" Then
            ...
            'Open other page.
            Response.Redirect($"~/FORMS/BAJET/SEMAKAN BAJET/MaklumatSemakanBajet_Bend_RV.aspx?KodSubMenu={KodSubMenu}&no={NoMohon}")
        ...
        ElseIf e.CommandName = "10Op" Or e.CommandName = "10Ko" Or e.CommandName = "20Op" Or e.CommandName = "20Ko" Or e.CommandName = "30Op" Or e.CommandName = "30Ko" Or e.CommandName = "40Op" Or e.CommandName = "40Ko" Or e.CommandName = "50Op" Or e.CommandName = "50Ko" Then
            ...
            Try
                ...
                If lblCheck <> "0.00" Then
                    ...
                    'Open other page.
                    Response.Redirect($"~/FORMS/BAJET/SEMAKAN BAJET/Maklumat_Agihan_Bajet_Review_Bend.aspx?KodSub=" & strKodSub & "&KodSubMenu=" & strKodSubMenu & "&KodPTj=" & strKodPTj & "&Tahun=" & strTahun & "&KodKW=" & strKodKW & "&kodVot=" & strKodVot & "&strKodKO=" & strKodKO & "", False)
        ...
        Else
            ...
            For Each row As GridViewRow In rows
                ...
                If chk.Checked Then
                    ...
                    strSqlButiran = "Update BG01_Mohon_RV set StatusDok = @statusDok , BG20_AmaunLulusNC = @idSemak ,BG20_TarikhLulusNC = @tarikh   " + flagKB + " WHERE BG20_Status = @status and BG20_NoMohon = @NoMohon"
                    paramSqlBtrn = {
            ...
            If countButiran > 0 Then
                fGlobalAlert($"Berjaya simpan", Me.Page, Me.[GetType](), "../../BAJET/SEMAKAN BAJET/Penyediaan_Bendahari.aspx")
            Else
    ...
    Protected Sub lblKemaskiniVot_Click(sender As Object, e As EventArgs) Handles lblKemaskiniVot.Click
        ...
        If dbconn_.fUpdateCommand(strSQL_Save_, paramSql_simpan_) > 0 Then
            ...
            If countButiran > 0 Then
                fGlobalAlert($"Maklumat berjaya dikemaskini.", Me.Page, Me.[GetType](), "../../BAJET/SEMAKAN BAJET/Penyediaan_Bendahari.aspx")
            Else
                fGlobalAlert("Maklumat Butiran tidak berjaya disimpan!", Me.Page, Me.[GetType]())
            End If
    ...
    Private Sub gvMohonBajet_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gvMohonBajet.SelectedIndexChanged
        ...
        'Open other page.
        Response.Redirect($"~/FORMS/BAJET/SEMAKAN BAJET/MaklumatSemakanBajet_Bend_RV.aspx?KodSub={KodSub}&KodSubMenu={KodSubMenu}&kodSkrin=1&no={strNoMohon}", False)
    End Sub
