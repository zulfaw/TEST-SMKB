...
Public Class Maklumat_Agihan_Bajet_Am_NC_RV
...
group by KodKO"
    ...
    Private Sub lnkBtnBack_Click(sender As Object, e As EventArgs) Handles lnkBtnBack.Click
        ...
        'Open other page.
        Response.Redirect($"~/FORMS/BAJET/SEMAKAN BAJET/Pengesahan_NC.aspx?KodSub={KodSub}&KodSubMenu={KodSubMenu}", False)
    End Sub
    ...
    Private Function fBindGvObjSebagai(strTahun, strKodKW, strKodKO, strKodPTj, strKodKP, strKodVot)
        ...
        case when ISNULL(SUM(am.JumlahNC ),0) = 0 then
        ISNULL(SUM(BG20_JumNC),0) 
        else
        ISNULL(SUM(am.JumlahNC ),0)
        end as jumlahNC
        from BG01_MohonDt_RV as a
...
group by KodKP"
    ...
    Private Sub gvButiran_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gvButiran.SelectedIndexChanged
        ...
        'Open other page.
        Response.Redirect($"~/FORMS/BAJET/SEMAKAN BAJET/MaklumatSemakanBajet_NC_RV.aspx?KodSub={KodSub}&KodSubMenu={KodSubMenu}&kodSkrin=2&no={strNoMohon}&kodvot={kodVot}", False)
    End Sub
    ...
    Protected Sub lnlBtnSave_Click(sender As Object, e As EventArgs) Handles lnlBtnSave.Click
        ...
        If chkExist = "N" Then
            ...
            Dim strSQLStatusDok = "INSERT INTO BG01_JumlahBajetAm (BG20_TahunBajet , KodPtj,  KodKW, KodOperasi, KodVotAm, JumlahNC, JumlahLPU) " &
                           " VALUES (@tahun, @ptj, @kw, @ko, @vot, @jumlah, @jumlah)"
        ...
        Else
            Dim strSQLStatusDok = "Update BG01_JumlahBajetAm set JumlahNC =@jumlah, JumlahLPU = @jumlah
    ...
    Private Sub fJumAll()
        ...
        Try
            ...
            If ds_.Tables.Count > 0 Then
                If ds_.Tables(0).Rows.Count = 1 Then
                    Dim jumlah = ds_.Tables(0).Rows(0)("JumlahNC").ToString
                    txtJum.Text = jumlah
                ...
                Else
                    'baca sum dari dt
                    Dim strSql2 As String = $"select ISNULL(SUM(BG20_JumNC),0) as jumlahNC from BG01_MohonDt_RV as a
                inner join  BG01_Mohon_RV as b on a.BG20_NoMohon = b.BG20_NoMohon
                inner join MK_Vot as c on c.kodvot = a.KodVotSebagai and c.status = 1
                WHERE b.BG20_TahunBajet = '{tahun}' and b.KodKW = '{kw}' and b.KodOperasi = '{ko}' and b.KodPtj = '{Ptj}' and a.KodVotAm = '{votAm}' and BG20_Aktif = '1' "
                    ...
                    If ds2_.Tables.Count > 0 Then
                        If ds2_.Tables(0).Rows.Count = 1 Then
                            Dim jumlah = ds2_.Tables(0).Rows(0)("JumlahNC").ToString
                            txtJum.Text = jumlah
    ...
    Private Sub fJumAll_Sbg(strKodVotSbg)
        ...
        Try
            ...
            If ds_.Tables.Count > 0 Then
                ...
                Else
                    'baca sum dari dt
                    Dim strSql2 As String = $"select ISNULL((BG20_JumNC),0) as JumlahNC  from BG01_MohonDt_RV as a
                    inner join  BG01_Mohon_RV as b on a.BG20_NoMohon = b.BG20_NoMohon
    ...
    Protected Sub lnlBtnSaveSbg_Click(sender As Object, e As EventArgs) Handles lnlBtnSaveSbg.Click
        ...
        If chkExist = "N" Then
            ...
            Dim strSQLStatusDok = "INSERT INTO BG01_JumlahBajetSbg (BG20_TahunBajet , KodPtj,  KodKW, KodOperasi, KodVotAm, KodVotSbg, JumlahNC, JumlahLPU) " &
                           " VALUES (@tahun, @ptj, @kw, @ko, @vot, @votSbg, @jumlah, @jumlah)"
            ...
            Response.Redirect($"~/FORMS/BAJET/SEMAKAN BAJET/Maklumat_Agihan_Bajet_Am_NC_RV.aspx?KodSub=0113&KodSubMenu=010110&KodPTj=" & Ptj & "&Tahun=" & tahun & "&KodKW=" & kw & "&kodVot=" & votAm & "&strKodKO=" & ko & "", False)


        Else
            Dim strSQLStatusDok = "Update BG01_JumlahBajetSbg set JumlahNC =@jumlah, JumlahLPU = @jumlah
