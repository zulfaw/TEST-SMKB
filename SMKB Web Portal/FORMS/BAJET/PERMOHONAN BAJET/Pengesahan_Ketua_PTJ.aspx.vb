...
AND KodBahagian = '4101')
    ...
    Private Sub fBindGVMohonBajet(KodStatus As String)
        Try
            ...
            If KodStatus = String.Empty Then
                strSql = $"select DISTINCT a.BG20_NoMohon, BG20_TarikhMohon, BG20_Program, ISNULL(( BG20_AmaunMohon_KetuaPTJ),0) AS BG20_AmaunMohon, StatusDok, KodKW, KodOperasi, BG20_Justifikasi from BG01_Mohon as a WHERE a.BG20_Status = 1 and B20_FlagKew = '1' " + ayatSql + " ORDER BY BG20_NoMohon DESC,  BG20_TarikhMohon DESC"
                strSql_count = $"select count(DISTINCT a.BG20_NoMohon) as bil  from BG01_Mohon as a WHERE a.BG20_Status = 1 and B20_FlagKew = '1'  " + ayatSql

            Else
                strSql = $"select DISTINCT a.BG20_NoMohon, BG20_TarikhMohon, BG20_Program, ISNULL(( BG20_AmaunMohon_KetuaPTJ),0) AS BG20_AmaunMohon, StatusDok, KodKW, KodOperasi, BG20_Justifikasi from BG01_Mohon as a where StatusDok='{KodStatus}' and B20_FlagKew = '1' " + ayatSql + " ORDER BY BG20_NoMohon DESC,  BG20_TarikhMohon DESC"
                strSql_count = $"select count(DISTINCT a.BG20_NoMohon) as bil  from BG01_Mohon as a WHERE  StatusDok='{KodStatus}'  and a.BG20_Status = 1 and B20_FlagKew = '1'  " + ayatSql
    ...
    Protected Sub gvAgihPTj_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvAgihPTj.RowCommand
        ...
        If e.CommandName = "Select" Then
            ...
            'Open other page.
            Response.Redirect($"~/FORMS/BAJET/PERMOHONAN BAJET/MaklumatSemakanBajet_KetuaPtj.aspx?KodSubMenu={KodSubMenu}&no={NoMohon}")
        ...
        ElseIf e.CommandName = "10Op" Or e.CommandName = "10Ko" Or e.CommandName = "20Op" Or e.CommandName = "20Ko" Or e.CommandName = "30Op" Or e.CommandName = "30Ko" Or e.CommandName = "40Op" Or e.CommandName = "40Ko" Or e.CommandName = "50Op" Or e.CommandName = "50Ko" Then
            ...
            Try
                ...
                If lblCheck <> "0.00" Then
                    ...
                    'Open other page.
                    Response.Redirect($"~/FORMS/BAJET/PERMOHONAN BAJET/Maklumat_Agihan_Bajet_Am_KetuaPTj.aspx?KodSub=" & strKodSub & "&KodSubMenu=" & strKodSubMenu & "&KodPTj=" & strKodPTj & "&Tahun=" & strTahun & "&KodKW=" & strKodKW & "&kodVot=" & strKodVot & "&strKodKO=" & strKodKO & "", False)
    ...
    Protected Sub lbtnHantar_Click(sender As Object, e As EventArgs) Handles lbtnHantar.Click
        ...
        Else
            ...
            If ddlStatus = "1" Then
                statusDok = "03"
                flagKB = " ,  B20_FlagPTj = '1'"
            ElseIf ddlStatus = "0" Then
                statusDok = "06"
                flagKB = " ,  BG20_FlagPemohon = '0', BG20_FlagKB = '0', B20_FlagKew = '0', B20_FlagPTj = '0' "
            End If
            ...
            For Each row As GridViewRow In rows
                ...
                If chk.Checked Then
                    ...
                    strSqlButiran = "Update BG01_Mohon set StatusDok = @statusDok , BG20_StaffIDPelulus = @idSemak ,BG20_TarikhSemak = @tarikh ,  BG20_UlasanPTj = @ulasan   " + flagKB + "   WHERE BG20_Status = @status and BG20_NoMohon = @NoMohon"
                    paramSqlBtrn = {
            ...
            If countButiran > 0 Then
                fGlobalAlert($"Berjaya simpan", Me.Page, Me.[GetType](), "../../BAJET/PERMOHONAN BAJET/Pengesahan_Ketua_PTJ.aspx")
            Else
    ...
    Protected Sub lblKemaskiniVot_Click(sender As Object, e As EventArgs) Handles lblKemaskiniVot.Click
        ...
        If dbconn_.fUpdateCommand(strSQL_Save_, paramSql_simpan_) > 0 Then
            ...
            If countButiran > 0 Then
                fGlobalAlert($"Maklumat berjaya dikemaskini.", Me.Page, Me.[GetType](), "../../BAJET/PERMOHONAN BAJET/Pengesahan_Ketua_PTJ.aspx")
            Else
                fGlobalAlert("Maklumat Butiran tidak berjaya disimpan!", Me.Page, Me.[GetType]())
            End If
    ...
    Private Sub gvMohonBajet_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gvMohonBajet.SelectedIndexChanged
        ...
        'Open other page.
        Response.Redirect($"~/FORMS/BAJET/PERMOHONAN BAJET/MaklumatSemakanBajet_KetuaPtj.aspx?KodSub={KodSub}&KodSubMenu={KodSubMenu}&kodSkrin=1&no={strNoMohon}", False)
    End Sub
    ...
    Protected Sub lblCetak_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lblCetak.Click
        ...
        Dim kodkw = ddlKW.SelectedValue
        Dim namaLaporan = "BG_PermohonanByKetuaPtj&kodkw=" + kodkw + "&kodptj=" + kodptj + "&ptj=" + kodptj
        Dim url = namaLaporan
    ...
    Protected Sub lblCetakDt_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lblCetakDt.Click
        ...
        Dim kodkw = ddlKW.SelectedValue
        Dim namaLaporan = "BG_PermohonanDt_KetuaPTj&kodkw=" + kodkw + "&kodptj=" + kodptj + "&ptj=" + kodptj
        Dim url = namaLaporan
        Dim strSVR = $"https://reporting.utem.edu.my/ReportServer/Pages/ReportViewer.aspx?%2fS2%2fS2D%2f{url}"
        Dim fullURL As String = $"window.open('{strSVR}', '_blank', 'width=' + (parseInt(window.innerWidth) * 0.3) + ',height=' + (parseInt(window.innerHeight) * .3) + ',status=yes,toolbar=no,menubar=no,location=no,scrollbars=yes,resizable=yes,titlebar=no' );"
        ScriptManager.RegisterStartupScript(Me, GetType(String), "OPEN_WINDOW", fullURL, True)
