...
Public Class MaklumatSemakanBajet_KewPTJ
    ...
    Protected Sub gvButiran_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvButiran.RowCommand
        ...
        If e.CommandName = "Padam" Then
            If noMohon <> "" Then
                ...
                Dim strSQL_ = $"UPDATE 
                                BG01_MohonDt SET  BG20_FlagKewPTj_Delete = @Status Where BG20_NoMohon = '{noMohon}' and BG20_Id = '{idDt}'"
    ...
    Private Sub MohonEdit(NoMohon As String)
        ...
        Try
            ...
            Dim strSql = $"Select KodPtj,KodBahagian,KodUnitPtj,KodMaksud,KodDasar,KodKW, BG20_TarikhMohon,BG20_TahunBajet,
                            KodOperasi,BG20_StaffIDPemohon,BG20_StaffIDPenyokong,BG20_StaffIDPengesah,StatusDok,CASE WHEN BG20_AmaunMohon_BajetPTJ IS NOT NULL  THEN BG20_AmaunMohon_BajetPTJ ELSE BG20_AmaunMohon END AS BG20_AmaunMohon,
            ...
            Dim pemohonBajet = dsMohon.Tables(0).Rows(0)("BG20_StaffIDPemohon").ToString
            ...
            If Not String.IsNullOrEmpty(pemohonBajet) Then

                dtUserInfo = fGetUserInfo(pemohonBajet)

                If dtUserInfo.Rows.Count > 0 Then
                    txtNamaPemohon.Text = dtUserInfo.Rows.Item(0).Item("MS01_Nama")
                    txtJawPemohon.Text = dtUserInfo.Rows.Item(0).Item("JawGiliran")
                End If
            End If
            ...
            Dim strSqlDt = $"Select BG20_Id, BG20_NoButiran,BG20_Butiran,BG20_Kuantiti,CASE WHEN BG20_JumKewPTj IS NOT NULL  THEN ISNULL((BG20_JumKewPTj),0)  ELSE ISNULL((BG20_AngJumlah),0)  END AS BG20_AmaunMohon,BG20_AngJumlah,KodVotSebagai,KodVotAm, BG20_Butiran from BG01_MohonDt where BG20_NoMohon='{NoMohon}' and BG20_Aktif = '1'"
    ...
    Protected Sub lbtnKembali_Click(sender As Object, e As EventArgs) Handles lbtnKembali.Click
        ...
        Dim strKodSub = Request.QueryString("KodSub")
        Dim strKodSubMenu = Request.QueryString("KodSubMenu")
        Dim strKodPTj = lblBahagian.Text
        ...
        Else
            Response.Redirect("~/FORMS/BAJET/PERMOHONAN BAJET/Semakan_Bajet_PTJ.aspx")
        End If
    ...
    Protected Sub lbtnHapus_Click(sender As Object, e As EventArgs) Handles lbtnHapus.Click
        ...
        If countButiran > 0 Then
            fGlobalAlert($"Rekod telah dipadam!", Me.Page, Me.[GetType](), "../../BAJET/PERMOHONAN BAJET/Semakan_Bajet_PTJ.aspx")
        Else
        ...
        Try
            ...
            If Page.IsValid Then
                ...
                If txtNoMohon.Text IsNot String.Empty Then
                    ...
                    KodKW = @kodkw, BG20_Justifikasi = @justifikasi,  BG20_AmaunMohon_KetuaPTJ = @amaun, BG20_AmaunMohon_BajetPTJ = @amaun, BG20_AmaunBajet = @amaun Where BG20_NoMohon = '{noPermohonan}'"
                        ...
                        Dim strSQL1_ = $"UPDATE 
                                BG01_MohonDt SET  BG20_Aktif = @Status Where BG20_NoMohon = '{noPermohonan}' and BG20_FlagKewPTj_Delete = '0' "
                        ...
                        countButiran = 0
                            ...
                            If IsDBNull(drCurrentRow("ID")) Then
                                ...
                                strSqlButiran = "INSERT INTO BG01_MohonDt ( BG20_NoButiran, BG20_NoMohon, KodVotAm, KodVotSebagai, BG20_JumKewPTj, BG20_JumKetuaPTj, BG20_JumBendahari, BG20_JumNC, BG20_AngJumlah,  BG20_Butiran, BG20_Aktif)
                                            VALUES (@NoButiran, @NoMohon ,@KodVotAm, @KodVotSbg, @AngJumlah,@AngJumlah,@AngJumlah,@AngJumlah, @AngJumlah,  @Butiran, @Aktif )"
                                paramSqlBtrn = {
                            ...
                            Else
                                ...
                                BG20_AngJumlah= @AngJumlah,  BG20_JumKewPTj = @AngJumlah, BG20_JumKetuaPTj= @AngJumlah, BG20_JumBendahari= @AngJumlah, BG20_JumNC= @AngJumlah, KodVotAm = @KodVotAm, KodVotSebagai = @KodVotSbg  Where BG20_NoMohon = '{noPermohonan}' and BG20_Id = '{drCurrentRow("ID")}'"
                        ...
                        If countButiran > 0 Then
                            If lblKodSkrin.Text = "2" Then
                                ...
                                fGlobalAlert($"Rekod baru telah ditambah bersama '{countButiran.ToString}' butiran permohonan!", Me.Page, Me.[GetType](), $"../../BAJET/PERMOHONAN BAJET/MaklumatSemakanBajet_KewPtj.aspx?KodSub={KodSub}&KodSubMenu={KodSubMenu}&kodSkrin=2&no={strNoMohon}&kodvot={kodVot}")

                            Else

                                fGlobalAlert($"Rekod baru telah ditambah bersama '{countButiran.ToString}' butiran permohonan!", Me.Page, Me.[GetType](), "../../BAJET/PERMOHONAN BAJET/Semakan_Bajet_PTJ.aspx")
                        ...
                        Dim paramSql2() As SqlParameter =
                  {
                     ...
                     New SqlParameter("@Ulasan", "-")
                        ...
                        If dbconn.fInsertCommand(strSql, paramSql) > 0 Then
                            ...
                            If countButiran > 0 Then
                                fGlobalAlert($"Rekod telah dikemaskini bersama '{countButiran.ToString}' butiran permohonan!", Me.Page, Me.[GetType](), "../../BAJET/PERMOHONAN BAJET/Semakan_Bajet_PTJ.aspx")
                            Else
                                fGlobalAlert("Maklumat Butiran tidak berjaya disimpan!", Me.Page, Me.[GetType]())
                            End If
