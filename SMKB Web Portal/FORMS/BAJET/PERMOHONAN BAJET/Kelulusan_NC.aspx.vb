...
AND KodBahagian = '4101')
    ...
    Private Function fGetAmtVotAm(strTahun, strKodKW, strKO, strKodPTj, strVotAm) As Decimal
        Try
            ...
            strSql = $"select isnull(sum(b.BG20_JumNC),0) as BG06_Amaun from BG01_Mohon as a inner join BG01_Mohondt as b on a.BG20_NoMohon = b.BG20_NoMohon where a.BG20_TahunBajet = '{strTahun}' and a.KodKW = '{strKodKW}' and a.KodOperasi = '{strKO}' and a.KodPtj = '{strKodPTj}' and b.KodVotAm = '{strVotAm}' and b.BG20_Status ='1'  and BG20_FlagBend = '1'"
...
where MK01_Tahun = '" & Now.Year & "' and MK01_VotTahun.KodKw = '" & strKodKW & "' order by MK_PTJ .KodPTJ"
    ...
    Private Sub fBindGVMohonBajet(KodStatus As String)
        Try
            ...
            If KodStatus = String.Empty Then
                strSql = $"select DISTINCT a.BG20_NoMohon, BG20_TarikhMohon, BG20_Program, ISNULL((BG20_AmaunBajet),0) as BG20_AmaunMohon, StatusDok, KodKW, KodOperasi, BG20_Justifikasi from BG01_Mohon as a WHERE a.BG20_Status = 1 and BG20_FlagBend = '1' " + ayatSql + "  ORDER BY BG20_NoMohon DESC,  BG20_TarikhMohon DESC"
                strSql_count = $"select count(DISTINCT a.BG20_NoMohon) as bil  from BG01_Mohon as a WHERE a.BG20_Status = 1 and BG20_FlagBend = '1'  " + ayatSql
            Else
                strSql = $"select DISTINCT a.BG20_NoMohon, BG20_TarikhMohon, BG20_Program, ISNULL((BG20_AmaunBajet),0) as BG20_AmaunMohon, StatusDok, KodKW, KodOperasi, BG20_Justifikasi from BG01_Mohon as a where StatusDok='{KodStatus}' and BG20_FlagBend = '1' " + ayatSql + "  ORDER BY BG20_NoMohon DESC,  BG20_TarikhMohon DESC"
                strSql_count = $"select count(DISTINCT a.BG20_NoMohon) as bil  from BG01_Mohon as a WHERE StatusDok='{KodStatus}' and a.BG20_Status = 1 and BG20_FlagBend = '1'  " + ayatSql
            End If
        ...
        ElseIf e.CommandName = "10Op" Or e.CommandName = "10Ko" Or e.CommandName = "20Op" Or e.CommandName = "20Ko" Or e.CommandName = "30Op" Or e.CommandName = "30Ko" Or e.CommandName = "40Op" Or e.CommandName = "40Ko" Or e.CommandName = "50Op" Or e.CommandName = "50Ko" Then
            ...
            Try
                ...
                If lblCheck <> "0.00" Then
                    ...
                    'Open other page.
                    Response.Redirect($"~/FORMS/BAJET/PERMOHONAN BAJET/Maklumat_Agihan_Bajet_Am_NC.aspx?KodSub=" & strKodSub & "&KodSubMenu=" & strKodSubMenu & "&KodPTj=" & strKodPTj & "&Tahun=" & strTahun & "&KodKW=" & strKodKW & "&kodVot=" & strKodVot & "&strKodKO=" & strKodKO & "", False)
    ...
    Protected Sub lbtnHantar_Click(sender As Object, e As EventArgs) Handles lbtnHantar.Click
        ...
        Else
            ...
            If ddlStatus = "1" Then
                statusDok = "21"
                flagKB = " ,  BG20_FlagNC = '1',  BG20_FlagLPU = '1'"
            ElseIf ddlStatus = "0" Then
                statusDok = "23"
                flagKB = " ,  BG20_FlagPemohon = '0', BG20_FlagKB = '0', B20_FlagKew = '0', B20_FlagPTj = '0', BG20_FlagBend = '0', BG20_FlagNC = '0' "
            End If
            ...
            For Each row As GridViewRow In rows
                ...
                If chk.Checked Then
                    ...
                    strSqlButiran = "Update BG01_Mohon set StatusDok = @statusDok , BG20_StaffIDNC = @idSemak ,BG20_TarikhLulusNC = @tarikh ,  BG20_UlasanNC = @ulasan   " + flagKB + "   WHERE BG20_Status = @status and BG20_NoMohon = @NoMohon "
                    paramSqlBtrn = {
