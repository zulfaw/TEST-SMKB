    ...
    Protected Sub gvButiran_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvButiran.RowCommand
        ...
        If e.CommandName = "Padam" Then
            If noMohon <> "" Then
                ...
                Dim strSQL_ = $"UPDATE 
                                BG01_MohonDt SET  BG20_FlagKB_Delete = @Status Where BG20_NoMohon = '{noMohon}' and BG20_Id = '{idDt}'"
    ...
    Private Sub MohonEdit(NoMohon As String)
        ...
        Try
            ...
            Dim strSql = $"Select KodPtj,KodBahagian,KodUnitPtj,KodMaksud,KodDasar,KodKW, BG20_TarikhMohon,BG20_TahunBajet,
                            KodOperasi,BG20_StaffIDPemohon,BG20_StaffIDPenyokong,BG20_StaffIDPengesah,StatusDok,CASE WHEN BG20_AmaunMohon_KB IS NOT NULL  THEN BG20_AmaunMohon_KB ELSE BG20_AmaunMohon END AS BG20_AmaunMohon,
            ...
            Dim strSqlDt = $"Select BG20_Id, BG20_NoButiran,BG20_Butiran,BG20_Kuantiti,CASE WHEN BG20_JumKB IS NOT NULL  THEN BG20_JumKB ELSE BG20_AngHrgUnit END AS BG20_AmaunMohon,BG20_AngJumlah,KodVotSebagai,KodVotAm, BG20_Butiran from BG01_MohonDt where BG20_NoMohon='{NoMohon}' and BG20_Aktif = '1'"
        ...
        Try
            ...
            If Page.IsValid Then
                ...
                If dtCurrentTable.Rows(0)(1).ToString Is String.Empty Then
                    ...
                    BG01_Mohon SET KodDasar=@dasar, KodOperasi =@operasi, BG20_Program = @Program,
                    KodKW = @kodkw, BG20_Justifikasi = @justifikasi,   BG20_AmaunBajet = @amaun ,BG20_AmaunMohon_KB = @amaun, BG20_StaffIDPenyokong = @penyokong Where BG20_NoMohon = '{noPermohonan}'"
                        ...
                        Dim paramSql_simpan() As SqlParameter =
                         ...
                         New SqlParameter("@justifikasi", txtJust.Text.Replace("'", "\")),
                         New SqlParameter("@amaun", ViewState("TotalAmount")),
                         New SqlParameter("@penyokong", Session("ssusrID"))
                         }
                        ...
                        ' Dim strSQL_Delete = $"DELETE FROM BG01_MohonDt Where BG20_NoMohon = @nomohon and BG20_FlagKB_Delete = '0'"
                        'Dim paramSql_delete() As SqlParameter =
                        ' {
                        ...
                        Dim strSQL1_ = $"UPDATE 
                                BG01_MohonDt SET  BG20_Aktif = @Status Where BG20_NoMohon = '{noPermohonan}' and BG20_FlagKB_Delete = '0' "
                        ...
                        countButiran = 0
                            ...
                            If IsDBNull(drCurrentRow("ID")) Then
                                ...
                                strSqlButiran = "INSERT INTO BG01_MohonDt ( BG20_NoButiran, BG20_NoMohon, KodVotAm, KodVotSebagai,  BG20_AngJumlah, BG20_JumKB, BG20_JumKewPTj, BG20_JumKetuaPTj, BG20_JumBendahari, BG20_JumNC,  BG20_Butiran, BG20_Aktif)
                                            VALUES (@NoButiran, @NoMohon ,@KodVotAm, @KodVotSbg,  @AngJumlah, @AngJumlah,  @AngJumlah,  @AngJumlah,  @AngJumlah,  @AngJumlah,  @Butiran, @Aktif )"
                                paramSqlBtrn = {
                            ...
                            Else
                                ...
                                BG20_JumKB = @AngJumlah, BG20_JumKewPTj= @AngJumlah, BG20_JumKetuaPTj= @AngJumlah, BG20_JumBendahari= @AngJumlah, BG20_JumNC= @AngJumlah, KodVotAm = @KodVotAm, KodVotSebagai = @KodVotSbg  Where BG20_NoMohon = '{noPermohonan}' and BG20_Id = '{drCurrentRow("ID")}'"
                        ...
                        Next
                            ...
                            If countButiran > 0 Then
                                fGlobalAlert($"Rekod baru telah ditambah bersama '{countButiran.ToString}' butiran permohonan!", Me.Page, Me.[GetType](), "../../BAJET/PERMOHONAN BAJET/Semakan_Ketua_Bahagian.aspx")
                            Else
                    ...
                    Else
                        ...
                        strSql = "INSERT INTO BG01_MOHON (BG20_NoMohon,BG20_TarikhMohon,KodPtj,KodBahagian,KodUnitPtj,KodKorporat,
                            BG20_Program,BG20_Justifikasi,BG20_TahunBajet,KodDasar,KodOperasi,KodKW,BG20_Status,StatusDok,BG20_AmaunBajet, BG20_AmaunMohon, BG20_StaffIDPemohon)
                            VALUES (@NoMohon, @tarikhMohon, @KodPTJ, @KodBah, @KodUnit, @KodAgensi,
                            @Program, @Justification, @TahunBajet, @KodJenDsr, @KodOperasi, @KodKW, @Status,  @StatusDok, @AmaunMohon, @AmaunMohon, @idMohon )"
                        ...
                        Dim paramSql2() As SqlParameter =
                  {
                     ...
                     New SqlParameter("@Ulasan", "-")
                        ...
                        If dbconn.fInsertCommand(strSql, paramSql) > 0 Then
                            ...
                            If countButiran > 0 Then
                                fGlobalAlert($"Rekod telah dikemaskini bersama '{countButiran.ToString}' butiran permohonan!", Me.Page, Me.[GetType](), "../../BAJET/PERMOHONAN BAJET/Semakan_Ketua_Bahagian.aspx")
                            Else
                                fGlobalAlert("Maklumat Butiran tidak berjaya disimpan!", Me.Page, Me.[GetType]())
                            End If
