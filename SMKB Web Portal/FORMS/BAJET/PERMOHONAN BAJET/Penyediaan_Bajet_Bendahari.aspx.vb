...
Public Class Penyediaan_Bajet_Bendahari
...
AND KodBahagian = '4101')
    ...
    Private Function fGetAmtVotAm(strTahun, strKodKW, strKO, strKodPTj, strVotAm) As Decimal
        Try
            ...
            strSql = $"select isnull(sum(b.BG20_JumBendahari),0) as BG06_Amaun from BG01_Mohon as a inner join BG01_Mohondt as b on a.BG20_NoMohon = b.BG20_NoMohon where a.BG20_TahunBajet = '{strTahun}' and a.KodKW = '{strKodKW}' and a.KodOperasi = '{strKO}' and a.KodPtj = '{strKodPTj}' and b.KodVotAm = '{strVotAm}' and b.BG20_Status ='1'   and B20_FlagPTj = '1' "
...
where MK01_Tahun = '" & Now.Year & "' and MK01_VotTahun.KodKw = '" & strKodKW & "' order by MK_PTJ .KodPTJ"
    ...
    Private Sub fBindDdlKO()
        Try
            ...
            ddlKO.DataTextField = "Butiran2"
            ddlKO.DataValueField = "KodKO"
            ddlKO.DataBind()

            ddlKO.Items.Insert(0, "- SILA PILIH KOD OPERASI -")
    ...
    Private Sub fBindGVMohonBajet(KodStatus As String)
        Try
            ...
            If (kodko <> "") Then
                ayatSql = ayatSql + $" and KodOperasi = '{ddlKO.SelectedValue}' "
            End If

            Dim strSql As String = ""
            If KodStatus = String.Empty Then
                strSql = $"select DISTINCT a.BG20_NoMohon, BG20_TarikhMohon, BG20_Program, ISNULL((BG20_AmaunSyorBendahari),0) as BG20_AmaunMohon, StatusDok, KodKW, KodOperasi, BG20_Justifikasi from BG01_Mohon as a WHERE a.BG20_Status = 1 and B20_FlagPTj = '1' " + ayatSql + "  ORDER BY BG20_NoMohon DESC,  BG20_TarikhMohon DESC "
                strSql_count = $"select count(DISTINCT a.BG20_NoMohon) as bil  from BG01_Mohon as a WHERE a.BG20_Status = 1 and B20_FlagPTj = '1'  " + ayatSql
            Else
                strSql = $"select DISTINCT a.BG20_NoMohon, BG20_TarikhMohon, BG20_Program, ISNULL((BG20_AmaunSyorBendahari),0) as BG20_AmaunMohon, StatusDok, KodKW, KodOperasi, BG20_Justifikasi from BG01_Mohon as a where StatusDok='{KodStatus}' and B20_FlagPTj = '1' " + ayatSql + "  ORDER BY BG20_NoMohon DESC,  BG20_TarikhMohon DESC"
                strSql_count = $"select count(DISTINCT a.BG20_NoMohon) as bil  from BG01_Mohon as a WHERE  StatusDok='{KodStatus}'  and a.BG20_Status = 1 and B20_FlagPTj = '1'  " + ayatSql
            End If
    ...
    Protected Sub gvAgihPTj_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvAgihPTj.RowCommand
        ...
        If e.CommandName = "Select" Then
            ...
            'Open other page.
            Response.Redirect($"~/FORMS/BAJET/PERMOHONAN BAJET/MaklumatSemakanBajet_Bend.aspx?KodSubMenu={KodSubMenu}&no={NoMohon}")
    ...
    Protected Sub lbtnHantar_Click(sender As Object, e As EventArgs) Handles lbtnHantar.Click
        ...
        Else
            ...
            If ddlStatus = "1" Then
                statusDok = "05"
                flagKB = " ,  BG20_FlagBend = '1'"
            ElseIf ddlStatus = "0" Then
                statusDok = "07"
                flagKB = " ,  BG20_FlagPemohon = '0', BG20_FlagKB = '0', B20_FlagKew = '0', B20_FlagPTj = '0', BG20_FlagBend = '0' "
            End If
            ...
            For Each row As GridViewRow In rows
                ...
                If chk.Checked Then
                    ...
                    strSqlButiran = "Update BG01_Mohon set StatusDok = @statusDok , BG20_StaffIDBendahari = @idSemak ,BG20_TarikhLulusBend = @tarikh,  BG20_UlasanBend = @ulasan   " + flagKB + "    WHERE BG20_Status = @status and BG20_NoMohon = @NoMohon"
                    paramSqlBtrn = {
            ...
            If countButiran > 0 Then
                fGlobalAlert($"Berjaya simpan", Me.Page, Me.[GetType](), "../../BAJET/PERMOHONAN BAJET/Penyediaan_Bajet_Bendahari.aspx")
            Else
    ...
    Protected Sub lblKemaskiniVot_Click(sender As Object, e As EventArgs) Handles lblKemaskiniVot.Click
        ...
        If dbconn_.fUpdateCommand(strSQL_Save_, paramSql_simpan_) > 0 Then
            ...
            If countButiran > 0 Then
                fGlobalAlert($"Maklumat berjaya dikemaskini.", Me.Page, Me.[GetType](), "../../BAJET/PERMOHONAN BAJET/Penyediaan_Bajet_Bendahari.aspx")
            Else
                fGlobalAlert("Maklumat Butiran tidak berjaya disimpan!", Me.Page, Me.[GetType]())
            End If
    ...
    Private Sub gvMohonBajet_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gvMohonBajet.SelectedIndexChanged
        ...
        'Open other page.
        Response.Redirect($"~/FORMS/BAJET/PERMOHONAN BAJET/MaklumatSemakanBajet_Bend.aspx?KodSub={KodSub}&KodSubMenu={KodSubMenu}&kodSkrin=1&no={strNoMohon}", False)
    End Sub
    ...
    Protected Sub lblCetak_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lblCetak.Click
        ...
        Dim kodkw = ddlKW.SelectedValue
        Dim namaLaporan = "BG_PermohonanByBendahari&kodkw=" + kodkw + "&kodptj=" + kodptj + "&ptj=" + kodptj
        Dim url = namaLaporan
        Dim strSVR = $"http://reporting.utem.edu.my/ReportServer/Pages/ReportViewer.aspx?%2fS2%2fS2D%2f{url}"
