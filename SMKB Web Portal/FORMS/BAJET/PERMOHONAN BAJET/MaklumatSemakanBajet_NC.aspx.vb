...
Public Class MaklumatSemakanBajet_NC
    ...
    Protected Sub gvButiran_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvButiran.RowCommand
        ...
        If e.CommandName = "Padam" Then
            If noMohon <> "" Then
                ...
                Dim strSQL_ = $"UPDATE 
                                BG01_MohonDt SET  BG20_JumNC = @calc , BG20_JumLPU = @calc, BG20_FlagNC_Delete = @Status, BG20_FlagLPU_Delete = @Status Where BG20_NoMohon = '{noMohon}' and BG20_Id = '{idDt}'"
            ...
            'baca amaun latest
            Dim strSQL2_ = $"SELECT ISNULL(sum(ISNULL(BG20_JumNC,0)),0) as jumBend FROM BG01_MohonDt Where BG20_NoMohon = '{noMohon}' and BG20_FlagNC_Delete <> '0' and BG20_Status = '1'"
            ...
            If ds.Tables.Count > 0 Then
                ...
                Dim strSQL_Save = $"UPDATE 
                    BG01_Mohon SET   BG20_AmaunLulusNC =@amaun , BG20_AmaunLulusLPU =@amaun, BG20_AmaunBajet = @amaun Where BG20_NoMohon = '{noPermohonan}'"
    ...
    Private Sub firstBindViewStateButiran()
        Try
            ...
            dt.Columns.AddRange(New DataColumn(7) {
                            ...
                            New DataColumn("BendDelete", GetType(Decimal)),
    ...
    Private Sub MohonEdit(NoMohon As String)
        ...
        Try
            ...
            Dim strSql = $"Select KodPtj,KodBahagian,KodUnitPtj,KodMaksud,KodDasar,KodKW, BG20_TarikhMohon,BG20_TahunBajet,
                            KodOperasi,BG20_StaffIDPemohon,BG20_StaffIDPenyokong,BG20_StaffIDPengesah,StatusDok,CASE WHEN BG20_AmaunLulusNC IS NOT NULL  THEN BG20_AmaunLulusNC ELSE BG20_AmaunMohon END AS BG20_AmaunMohon,
                            BG20_Justifikasi,BG20_Program, BG20_AmaunMohon from BG01_Mohon where BG20_NoMohon='{NoMohon}'"
            ...
            Dim strSqlDt = $"Select BG20_Id, BG20_NoButiran,BG20_Butiran,BG20_Kuantiti, ISNULL((BG20_JumKetuaPTj),0)  as jumPTj, CASE WHEN BG20_JumNC IS NOT NULL  THEN ISNULL((BG20_JumNC),0) ELSE ISNULL((BG20_JumKetuaPTj),0) END AS BG20_AmaunMohon,BG20_AngJumlah,KodVotSebagai,KodVotAm, BG20_Butiran, isnull(BG20_FlagNC_Delete,1) as BendDelete from BG01_MohonDt where BG20_NoMohon='{NoMohon}' and BG20_Status = '1'"
            ...
            For Each rowdt In dsMohonDt.Tables(0).Rows
                ...
                Dim a = rowdt("BendDelete").ToString
                Dim b
                If a = False Then
                    b = "0"
                Else
    ...
    Protected Sub lbtnKembali_Click(sender As Object, e As EventArgs) Handles lbtnKembali.Click
        ...
        Else
            Response.Redirect("~/FORMS/BAJET/PERMOHONAN BAJET/Kelulusan_NC.aspx")
        End If
    ...
    Protected Sub lbtnHapus_Click(sender As Object, e As EventArgs) Handles lbtnHapus.Click
        ...
        If countButiran > 0 Then
            fGlobalAlert($"Rekod telah dipadam!", Me.Page, Me.[GetType](), "../../BAJET/PERMOHONAN BAJET/Kelulusan_NC.aspx")
        Else
        ...
        Try
            ...
            If Page.IsValid Then
                ...
                If txtNoMohon.Text IsNot String.Empty Then
                    ...
                    KodKW = @kodkw, BG20_Justifikasi = @justifikasi,  BG20_AmaunLulusNC = @amaun, BG20_AmaunBajet = @amaun Where BG20_NoMohon = '{noPermohonan}'"
                    ...
                    If dbconn.fUpdateCommand(strSQL_Save, paramSql_simpan) > 0 Then
                        ...
                        Dim strSQL1_ = $"UPDATE 
                                BG01_MohonDt SET  BG20_Aktif = @Status Where BG20_NoMohon = '{noPermohonan}' and BG20_FlagNC_Delete = '0' "
                        ...
                        For Each drCurrentRow As DataRow In dtCurrentTable.Rows
                            ...
                            If IsDBNull(drCurrentRow("ID")) Then
                                ...
                                strSqlButiran = "INSERT INTO BG01_MohonDt ( BG20_NoButiran, BG20_NoMohon, KodVotAm, KodVotSebagai,   BG20_JumNC, BG20_AngJumlah,  BG20_Butiran, BG20_Aktif)
                            ...
                            Else
                                ...
                                BG20_AngJumlah= @AngJumlah,  BG20_JumNC= @AngJumlah, KodVotAm = @KodVotAm, KodVotSebagai = @KodVotSbg  Where BG20_NoMohon = '{noPermohonan}' and BG20_Id = '{drCurrentRow("ID")}'"
                        ...
                        If countButiran > 0 Then
                            If lblKodSkrin.Text = "2" Then
                                ...
                                fGlobalAlert($"Rekod baru telah ditambah bersama '{countButiran.ToString}' butiran permohonan!", Me.Page, Me.[GetType](), $"../../BAJET/PERMOHONAN BAJET/MaklumatSemakanBajet_NC.aspx?KodSub={KodSub}&KodSubMenu={KodSubMenu}&kodSkrin=2&no={strNoMohon}&kodvot={kodVot}")

                            Else

                                fGlobalAlert($"Rekod baru telah ditambah bersama '{countButiran.ToString}' butiran permohonan!", Me.Page, Me.[GetType](), "../../BAJET/PERMOHONAN BAJET/Kelulusan_NC.aspx")
                ...
                Else
                    ...
                    Dim paramSql2() As SqlParameter =
                  {
                    ...
                    If dbconn.fInsertCommand(strSql, paramSql) > 0 Then
                        ...
                        If countButiran > 0 Then
                            fGlobalAlert($"Rekod telah dikemaskini bersama '{countButiran.ToString}' butiran permohonan!", Me.Page, Me.[GetType](), "../../BAJET/PERMOHONAN BAJET/Kelulusan_NC.aspx")
                        Else
                            fGlobalAlert("Maklumat Butiran tidak berjaya disimpan!", Me.Page, Me.[GetType]())
                        End If
    ...
    Private Sub gvButiran_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles gvButiran.RowDataBound
        Try
            If e.Row.RowType = DataControlRowType.DataRow Then
                ...
                Dim benddelete As String = Convert.ToString(DataBinder.Eval(e.Row.DataItem, "BendDelete").ToString())

                If benddelete Then
                    ' e.Row.Attributes.Add("style", "this.style.backgroundColor = '#00FF00';")
                    e.Row.BackColor = System.Drawing.Color.White
                    e.Row.FindControl("btnAktif").Visible = False
                    e.Row.FindControl("btnDelete").Visible = True
                Else

                    e.Row.BackColor = System.Drawing.Color.Tomato
                    e.Row.FindControl("btnAktif").Visible = True
