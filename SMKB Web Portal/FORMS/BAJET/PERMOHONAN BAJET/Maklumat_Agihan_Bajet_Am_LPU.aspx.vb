Public Class Maklumat_Agihan_Bajet_Am_LPU
...
group by KodKO"
    ...
    Private Sub lnkBtnBack_Click(sender As Object, e As EventArgs) Handles lnkBtnBack.Click
        ...
        'Open other page.
        Response.Redirect($"~/FORMS/BAJET/PERMOHONAN BAJET/Kelulusan_LPU.aspx?KodSub={KodSub}&KodSubMenu={KodSubMenu}", False)
    End Sub
    ...
    Private Function fBindGvObjSebagai(strTahun, strKodKW, strKodKO, strKodPTj, strKodKP, strKodVot)
        ...
        WHERE b.BG20_TahunBajet = '{strTahun}' and b.KodKW = '{strKodKW}' and b.KodOperasi = '{strKodKO}' and b.KodPtj = '{strKodPTj}' and a.KodVotAm = '{strKodVot}'  and a.BG20_Status ='1' and  BG20_FlagNC = '1'
        GROUP BY a.KodVotSebagai , c.Butiran Order by a.KodVotSebagai "
            ...
            If Not ds Is Nothing Then
                If ds.Tables(0).Rows.Count > 0 Then
                    ...
                    dt.Columns.Add("AmtAgihLPU", GetType(String))

                    Dim KodSbg As String
                    Dim decJumBajet As Decimal
                    Dim Butiran As String

                    Dim strAmtAgihPast, strAmtBelanjaPast, strAmtMohon, strAmtCad, strAmtLPU
                    ...
                    For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
                        ...
                        strAmtLPU = CDec(ds.Tables(0).Rows(i)("jumlahLPU"))

                        dt.Rows.Add(KodSbg, Butiran, FormatNumber(strAmtAgihPast), FormatNumber(strAmtBelanjaPast), FormatNumber(strAmtMohon), FormatNumber(strAmtCad), FormatNumber(decJumBajet), FormatNumber(strAmtLPU))
...
group by KodKP"
    ...
    Dim decTotAmtCad, decTotAmtAgih, decTotAmtMohon, decTotAmtBelanjaPast, decTotAmtLPU, decTotAmtAgihPast As Decimal
    Private Sub gvObjSebagai_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles gvObjSebagai.RowDataBound
        Try
            If e.Row.RowType = DataControlRowType.DataRow Then
                ...
                Dim lblAmtLPU As Label = CType(e.Row.FindControl("lblAmtLPU"), Label)
                decTotAmtLPU += CDec(lblAmtLPU.Text)

                Dim lblAmtNC As Label = CType(e.Row.FindControl("lblAmtNC"), Label)
                decTotAmtAgih += CDec(lblAmtNC.Text)
            ...
            If e.Row.RowType = DataControlRowType.Footer Then
                ...
                Dim lblTotAgihLPU As Label = CType(e.Row.FindControl("lblTotAgihLPU"), Label)
                lblTotAgihLPU.Text = FormatNumber(decTotAmtLPU)
    ...
    Private Sub sBindGvButiran(strKodVotSbg, strTahun, kodkw, kodKO, kodPtj)
        ...
        Try
            ...
            strSql = $"select a.BG20_NoMohon, a.KodVotSebagai as KodVot, b.BG20_Program, a.BG20_Butiran ,ISNULL((BG20_JumKetuaPTj),0) as BG20_JumKetuaPTj ,ISNULL((BG20_JumBendahari),0)  as BG20_JumBendahari,ISNULL((BG20_JumNC),0) as BG20_JumNC, ISNULL((BG20_JumLPU),0) as BG20_JumLPU, isnull(a.BG20_FlagLPU_Delete, 1) as BendDelete from BG01_MohonDt as a
                    inner join  BG01_Mohon as b on a.BG20_NoMohon = b.BG20_NoMohon
                    WHERE b.BG20_TahunBajet = '{strTahun}' and b.KodKW = '{kodkw}' and b.KodOperasi = '{kodKO}' and b.KodPTj = '{kodPtj}'
                    and a.KodVotSebagai = '{strKodVotSbg}' and a.BG20_Status ='1' and BG20_FlagLPU = '1' "
            ...
            If Not ds Is Nothing Then
                If ds.Tables(0).Rows.Count > 0 Then
                    ...
                    dt.Columns.Add("AmtAgihanLPU", GetType(String))
                    ...
                    Dim Program, AmtCad, AmtAgihan, AmtAgihanLPU
                    ...
                    For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
                        ...
                        AmtAgihanLPU = CDec(ds.Tables(0).Rows(i)("BG20_JumLPU"))
                        BendDelete = ds.Tables(0).Rows(i)("BendDelete")

                        dt.Rows.Add(Program, NoMohon, Butiran, FormatNumber(AmtMohon), FormatNumber(AmtCad), FormatNumber(AmtAgihan), FormatNumber(AmtAgihanLPU), BendDelete)
    ...
    Dim decTotAmtAgihBut, decTotAmtAgihMohon, decTotAmtAgihBend, decTotAmtAgihLPU As Decimal
    Private Sub gvButiran_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles gvButiran.RowDataBound
        Try
            If e.Row.RowType = DataControlRowType.DataRow Then
                ...
                Dim txtAmtAgihanLPU As Label = CType(e.Row.FindControl("lblAmtAgihanLPU"), Label)
                decTotAmtAgihLPU += CDec(txtAmtAgihanLPU.Text)
            ...
            If e.Row.RowType = DataControlRowType.Footer Then
                ...
                Dim lblTotAmtAgihanLPU As Label = CType(e.Row.FindControl("lblTotAmtAgihanLPU"), Label)
                lblTotAmtAgihanLPU.Text = FormatNumber(decTotAmtAgihLPU)
    ...
    Private Sub gvButiran_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gvButiran.SelectedIndexChanged
        ...
        'Open other page.
        Response.Redirect($"~/FORMS/BAJET/PERMOHONAN BAJET/MaklumatSemakanBajet_LPU.aspx?KodSub={KodSub}&KodSubMenu={KodSubMenu}&kodSkrin=2&no={strNoMohon}&kodvot={kodVot}", False)
    End Sub
