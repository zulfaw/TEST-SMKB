...
AND KodBahagian = '4101')
    ...
    Private Function fGetAmtVotAm(strTahun, strKodKW, strKO, strKodPTj, strVotAm) As Decimal
        Try
            ...
            strSql = $"select isnull(sum(b.BG20_JumLPU),0) as BG06_Amaun from BG01_Mohon as a inner join BG01_Mohondt as b on a.BG20_NoMohon = b.BG20_NoMohon where a.BG20_TahunBajet = '{strTahun}' and a.KodKW = '{strKodKW}' and a.KodOperasi = '{strKO}' and a.KodPtj = '{strKodPTj}' and b.KodVotAm = '{strVotAm}' and b.BG20_Status ='1' and BG20_FlagNC = '1'"
...
where MK01_Tahun = '" & Now.Year & "' and MK01_VotTahun.KodKw = '" & strKodKW & "' order by MK_PTJ .KodPTJ"
            Dim ds As New DataSet
            'Dim dbconn As New DBKewConn
            ds = dbconn.fSelectCommand(strSql)
        ...
        Try
            ...
            Dim strSql As String = "SELECT DISTINCT TOP 100 PERCENT dbo.MK_Kw.KodKw, (dbo.MK_Kw.KodKw + ' - ' + dbo.MK_Kw.Butiran) as Butiran " &
                  "FROM dbo.MK_Kw INNER JOIN dbo.MK01_VotTahun ON dbo.MK_Kw.KodKw = dbo.MK01_VotTahun.KodKw " &
                    "where dbo.MK01_VotTahun.MK01_Tahun = '" & Now.Year & "' ORDER BY dbo.MK_Kw.KodKw"
    ...
    Private Sub fBindGVMohonBajet(KodStatus As String)
        Try
            ...
            If KodStatus = String.Empty Then
                strSql = $"select DISTINCT a.BG20_NoMohon, BG20_TarikhMohon, BG20_Program, ISNULL((BG20_AmaunBajet),0) as BG20_AmaunMohon, StatusDok, KodKW, KodOperasi, BG20_Justifikasi from BG01_Mohon as a WHERE a.BG20_Status = 1 and BG20_FlagNC = '1' " + ayatSql + "  ORDER BY BG20_NoMohon DESC,  BG20_TarikhMohon DESC"
                strSql_count = $"select count(DISTINCT a.BG20_NoMohon) as bil  from BG01_Mohon as a WHERE a.BG20_Status = 1 and BG20_FlagNC = '1'  " + ayatSql
            Else
                strSql = $"select DISTINCT a.BG20_NoMohon, BG20_TarikhMohon, BG20_Program, ISNULL((BG20_AmaunBajet),0) as BG20_AmaunMohon, StatusDok, KodKW, KodOperasi, BG20_Justifikasi from BG01_Mohon as a where StatusDok='{KodStatus}' and BG20_FlagNC = '1' " + ayatSql + "  ORDER BY BG20_NoMohon DESC,  BG20_TarikhMohon DESC"
                strSql_count = $"select count(DISTINCT a.BG20_NoMohon) as bil  from BG01_Mohon as a WHERE StatusDok='{KodStatus}' and a.BG20_Status = 1 and BG20_FlagNC = '1'  " + ayatSql
            End If
    ...
    Protected Sub gvAgihPTj_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvAgihPTj.RowCommand
        ...
        If e.CommandName = "Select" Then
            ...
            'Open other page.
            Response.Redirect($"~/FORMS/BAJET/PERMOHONAN BAJET/MaklumatSemakanBajet_NC.aspx?KodSubMenu={KodSubMenu}&no={NoMohon}")
        ...
        ElseIf e.CommandName = "10Op" Or e.CommandName = "10Ko" Or e.CommandName = "20Op" Or e.CommandName = "20Ko" Or e.CommandName = "30Op" Or e.CommandName = "30Ko" Or e.CommandName = "40Op" Or e.CommandName = "40Ko" Or e.CommandName = "50Op" Or e.CommandName = "50Ko" Then
            ...
            Try
                ...
                If lblCheck <> "0.00" Then
                    ...
                    'Open other page.
                    Response.Redirect($"~/FORMS/BAJET/PERMOHONAN BAJET/Maklumat_Agihan_Bajet_Am_LPU.aspx?KodSub=" & strKodSub & "&KodSubMenu=" & strKodSubMenu & "&KodPTj=" & strKodPTj & "&Tahun=" & strTahun & "&KodKW=" & strKodKW & "&kodVot=" & strKodVot & "&strKodKO=" & strKodKO & "", False)
    ...
    Protected Sub lbtnHantar_Click(sender As Object, e As EventArgs) Handles lbtnHantar.Click
        ...
        Else
            ...
            ElseIf ddlStatus = "0" Then
                statusDok = "23"
                flagKB = " ,  BG20_FlagPemohon = '0', BG20_FlagKB = '0', B20_FlagKew = '0', B20_FlagPTj = '0', BG20_FlagBend = '0', BG20_FlagNC = '0', BG20_FlagLPU = '0'  "
            End If
            ...
            For Each row As GridViewRow In rows
                ...
                If chk.Checked Then
                    ...
                    strSqlButiran = "Update BG01_Mohon set StatusDok = @statusDok , BG20_StaffIDLPU = @idSemak ,BG20_TarikhLulusLPU = @tarikh ,  BG20_UlasanLPU = @ulasan   " + flagKB + "   WHERE BG20_Status = @status and BG20_NoMohon = @NoMohon "
                    paramSqlBtrn = {
                    ...
                    'transfer ke table review mohon dan dt sekiranya NC lulus
                    'mohon
                    Dim strSQLStatusDok_Review = "INSERT INTO BG01_Mohon_RV ( BG20_NoMohon, BG21_NoMohonPTj, KodPtj, KodBahagian, KodUnitPtj, KodMaksud, KodKorporat, BG20_Program, BG20_Justifikasi, BG20_TahunBajet, KodDasar, KodOperasi, KodKW, BG20_Status,
                    StatusDok, BG20_StaffIDPemohon, BG20_StaffIDPenyokong, BG20_AmaunMohon , BG20_AmaunMohon_KetuaPTJ , BG20_AmaunSyorBendahari ,BG20_AmaunBendahari, BG20_AmaunLulusNC, BG20_AmaunLPU, BG20_AmaunPelarasnPTj, BG20_TarikhMohon, BG20_TarikhLulusNC,  BG20_AmaunBajet) 
                    SELECT  BG20_NoMohon, BG21_NoMohonPTj, KodPtj, KodBahagian, KodUnitPtj, KodMaksud, KodKorporat, BG20_Program, BG20_Justifikasi, BG20_TahunBajet, KodDasar, KodOperasi, KodKW, BG20_Status,
                    @statusDok, BG20_StaffIDPemohon, BG20_StaffIDPenyokong, 
                    CASE 
	                    WHEN ISNULL(BG20_AmaunLulusLPU,0) = 0 THEN BG20_AmaunMohon
	                    ELSE BG20_AmaunMohon
	                    END
                    ,
                    CASE 
	                    WHEN ISNULL(BG20_AmaunLulusNC,0) = 0 THEN BG20_AmaunMohon
	                    ELSE BG20_AmaunMohon
	                    END
                    ,
                    CASE 
                    ...
                    ,CASE 
	                    WHEN ISNULL(BG20_AmaunLulusNC,0) = 0 THEN BG20_AmaunMohon
	                    ELSE BG20_AmaunLulusNC
	                    END, BG20_TarikhMohon, @tarikh,  BG20_AmaunBajet
                    FROM BG01_Mohon WHERE BG20_NoMohon = @NoMohon"

                    Dim paramSql2_Review() As SqlParameter =
                      {
                         New SqlParameter("@NoMohon", noMohon),
                         New SqlParameter("@statusDok", "23"),
                         New SqlParameter("@tarikh", tarikhMohon)
                         }

                    'mohondt
                    Dim strSQLStatusDok_ReviewDt = "INSERT  INTO BG01_MohonDt_RV (BG20_NoButiran, BG20_NoMohon, BG20_Butiran, BG20_Kuantiti, BG20_AngHrgUnit, BG20_AngJumlah, BG20_Status, KodVotSebagai, KodVotAm, BG20_JumKB, BG20_JumKetuaPTj, BG20_JumBendahari, BG20_JumNC, BG20_JumLPU, BG20_JumPelarasanPTj, BG20_Aktif) 
                    SELECT  BG20_NoButiran, BG20_NoMohon, BG20_Butiran, BG20_Kuantiti, BG20_JumNC, BG20_JumNC, BG20_Status, KodVotSebagai, KodVotAm, BG20_JumNC,BG20_JumNC,BG20_JumNC,BG20_JumNC,BG20_JumNC, BG20_JumNC,  BG20_Aktif
                    FROM BG01_MohonDt WHERE BG20_NoMohon = @NoMohon and BG20_Aktif = '1'"

                    Dim paramSql2_ReviewDt() As SqlParameter =
                      {
                         New SqlParameter("@NoMohon", noMohon)
                         }

                    'tamat proses transfer ke table review
                    ...
                    dbconn.fInsertCommand(strSQLStatusDok, paramSql2)
                    'INSERT KE REVIEW
                    dbconn.fInsertCommand(strSQLStatusDok_Review, paramSql2_Review)
                    dbconn.fInsertCommand(strSQLStatusDok_ReviewDt, paramSql2_ReviewDt)
            ...
            If countButiran > 0 Then
                fGlobalAlert($"Berjaya simpan", Me.Page, Me.[GetType](), "../../BAJET/PERMOHONAN BAJET/Kelulusan_NC.aspx")
            Else
    ...
    Private Sub fBindVot(NoMohon As String)
        Try
            ...
            strSql = $"select DISTINCT a.BG20_NoMohon, BG20_TarikhMohon, BG20_Program, BG20_AmaunMohon, StatusDok, KodKW, KodOperasi, BG20_Justifikasi from BG01_Mohon as a where a.BG20_NoMohon='{NoMohon}' "
            ...
            from BG01_MohonDt as a where a.BG20_NoMohon='{NoMohon}'
    ...
    Protected Sub lblKemaskiniVot_Click(sender As Object, e As EventArgs) Handles lblKemaskiniVot.Click
        ...
        Dim strSQL_Save_ = $"UPDATE BG01_Mohon SET BG20_AmaunMohon = @amaun Where BG20_NoMohon = @noMohon"
        ...
        If dbconn_.fUpdateCommand(strSQL_Save_, paramSql_simpan_) > 0 Then
            ...
            If countButiran > 0 Then
                fGlobalAlert($"Maklumat berjaya dikemaskini.", Me.Page, Me.[GetType](), "../../BAJET/PERMOHONAN BAJET/Kelulusan_NC.aspx")
            Else
                fGlobalAlert("Maklumat Butiran tidak berjaya disimpan!", Me.Page, Me.[GetType]())
            End If
    ...
    Private Sub gvMohonBajet_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gvMohonBajet.SelectedIndexChanged
        ...
        'Open other page.
        Response.Redirect($"~/FORMS/BAJET/PERMOHONAN BAJET/MaklumatSemakanBajet_NC.aspx?KodSub={KodSub}&KodSubMenu={KodSubMenu}&kodSkrin=1&no={strNoMohon}", False)
    End Sub
    ...
    '    Dim strSql As String
    '    Try
    '        strSql = "SELECT ISNULL(SUM(ISNULL(BG20_AmaunSyorBendahari,0)),0) as amaun FROM BG01_Mohon where BG20_FlagBend = '1'"
    '        Dim ds As New DataSet
    '        Dim dbconn As New DBKewConn
    '        Dim strAmaunMohon As String
    ...
    '    Dim strSql As String
    '    Try
    '        strSql = "SELECT isnull(SUM(ISNULL(BG20_AmaunLulusNC,0)),0) as amaun FROM BG01_Mohon where BG20_FlagNC = '1'"
    '        Dim ds As New DataSet
    '        Dim dbconn As New DBKewConn
    '        Dim strAmaunMohon As String
    ...
    Protected Sub lblCetak_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lblCetak.Click
        ...
        Dim strSql_id As String
        strSql_id = $"select att1 from live_user_staf where att15='{Session("ssusrID")}'"
        ...
        ds_id = dbconnEQ.fselectCommand(strSql_id)
        kodptj = ds_id.Tables(0).Rows(0).Item("att1").ToString
        ptj = ds_id.Tables(0).Rows(0).Item("att1").ToString

        Dim kodkw = ddlKW.SelectedValue
        Dim namaLaporan = "BG_PermohonanByNC&kodkw=" + kodkw + "&kodptj=" + kodptj + "&ptj=" + kodptj
        Dim url = namaLaporan
    ...
    Private Sub simpanPath(Path As String, FileName As String, ContentType As String, JenisLamp As String, noMohon As String)
        ...
        Try
            ...
            where BG01_NoMohon = '1' and BG16_JenisDok = 'Minit'"
    ...
    Private Sub fLoadGVSalinanBajet()
        Dim str = $"select BG16_Id, BG01_NoMohon, BG16_NamaDok, BG16_Path from BG16_Lampiran 
            where BG01_NoMohon = '1' and BG16_Status = 1  and BG16_JenisDok = 'Minit'"
