...
Public Class MaklumatSemakanBajet_KetuaPtj
    ...
    Private Sub MohonEdit(NoMohon As String)
        ...
        Try
            ...
            Dim strSql = $"Select KodPtj,KodBahagian,KodUnitPtj,KodMaksud,KodDasar,KodKW, BG20_TarikhMohon,BG20_TahunBajet,
                            KodOperasi,BG20_StaffIDPemohon,BG20_StaffIDPenyokong,BG20_StaffIDPengesah,StatusDok,CASE WHEN BG20_AmaunMohon_KetuaPTJ IS NOT NULL  THEN BG20_AmaunMohon_KetuaPTJ ELSE BG20_AmaunMohon END AS BG20_AmaunMohon,
            ...
            Dim strSqlDt = $"Select BG20_Id, BG20_NoButiran,BG20_Butiran,BG20_Kuantiti,CASE WHEN BG20_JumKetuaPTj IS NOT NULL  THEN ISNULL((BG20_JumKetuaPTj),0) ELSE ISNULL((BG20_AngJumlah),0) END AS BG20_AmaunMohon,BG20_AngJumlah,KodVotSebagai,KodVotAm, BG20_Butiran from BG01_MohonDt where BG20_NoMohon='{NoMohon}' and BG20_Aktif = '1'"
    ...
    Protected Sub lbtnHapus_Click(sender As Object, e As EventArgs) Handles lbtnHapus.Click
        ...
        If countButiran > 0 Then
            fGlobalAlert($"Rekod telah dipadam!", Me.Page, Me.[GetType](), "../../BAJET/PERMOHONAN BAJET/Pengesahan_Ketua_PTJ.aspx")
        Else
        ...
        Try
            ...
            If Page.IsValid Then
                ...
                If txtNoMohon.Text IsNot String.Empty Then
                    ...
                    KodKW = @kodkw, BG20_Justifikasi = @justifikasi,  BG20_AmaunMohon_KetuaPTJ = @amaun, BG20_AmaunBajet = @amaun Where BG20_NoMohon = '{noPermohonan}'"
                    ...
                    If dbconn.fUpdateCommand(strSQL_Save, paramSql_simpan) > 0 Then
                        ...
                        For Each drCurrentRow As DataRow In dtCurrentTable.Rows
                            ...
                            If IsDBNull(drCurrentRow("ID")) Then
                                ...
                                strSqlButiran = "INSERT INTO BG01_MohonDt ( BG20_NoButiran, BG20_NoMohon, KodVotAm, KodVotSebagai,  BG20_JumKetuaPTj, BG20_JumBendahari, BG20_JumNC, BG20_AngJumlah,  BG20_Butiran, BG20_Aktif)
                                            VALUES (@NoButiran, @NoMohon ,@KodVotAm, @KodVotSbg, @AngJumlah,@AngJumlah,@AngJumlah, @AngJumlah,  @Butiran, @Aktif )"
                                paramSqlBtrn = {
                            ...
                            Else
                                ...
                                BG20_AngJumlah= @AngJumlah,   BG20_JumKetuaPTj= @AngJumlah, BG20_JumBendahari= @AngJumlah, BG20_JumNC= @AngJumlah, KodVotAm = @KodVotAm, KodVotSebagai = @KodVotSbg  Where BG20_NoMohon = '{noPermohonan}' and BG20_Id = '{drCurrentRow("ID")}'"
                        ...
                        If countButiran > 0 Then
                            If lblKodSkrin.Text = "2" Then
                                ...
                                fGlobalAlert($"Rekod baru telah ditambah bersama '{countButiran.ToString}' butiran permohonan!", Me.Page, Me.[GetType](), $"../../BAJET/PERMOHONAN BAJET/MaklumatSemakanBajet_KetuaPtj.aspx?KodSub={KodSub}&KodSubMenu={KodSubMenu}&kodSkrin=2&no={strNoMohon}&kodvot={kodVot}")

                            Else

                                fGlobalAlert($"Rekod baru telah ditambah bersama '{countButiran.ToString}' butiran permohonan!", Me.Page, Me.[GetType](), "../../BAJET/PERMOHONAN BAJET/Pengesahan_Ketua_PTJ.aspx")
                ...
                Else
                    ...
                    Dim paramSql2() As SqlParameter =
                  {
                    ...
                    If dbconn.fInsertCommand(strSql, paramSql) > 0 Then
                        ...
                        If countButiran > 0 Then
                            fGlobalAlert($"Rekod telah dikemaskini bersama '{countButiran.ToString}' butiran permohonan!", Me.Page, Me.[GetType](), "../../BAJET/PERMOHONAN BAJET/Pengesahan_Ketua_PTJ.aspx")
                        Else
                            fGlobalAlert("Maklumat Butiran tidak berjaya disimpan!", Me.Page, Me.[GetType]())
                        End If
