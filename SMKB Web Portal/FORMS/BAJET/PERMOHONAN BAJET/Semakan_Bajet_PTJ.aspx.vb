...
Public Class Semakan_Bajet_PTJ
...
AND KodBahagian = '4101')
    ...
    Private Sub gvAgihPTj_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles gvAgihPTj.RowDataBound
        Try
            ...
            If e.Row.RowType = DataControlRowType.Footer Then
                ...
                lblAmtJumAll.Text = FormatNumber(decAmt10000Op + decAmt10000Ko + decAmt20000Op + decAmt20000Ko + decAmt30000Op + decAmt30000Ko + decAmt40000Op + decAmt40000Ko + decAmt50000Op + decAmt50000Ko)
            End If
    ...
    Private Function fGetAmtVotAm(strTahun, strKodKW, strKO, strKodPTj, strVotAm) As Decimal
        Try
            ...
            'strSql = $"select isnull(sum(b.BG20_JumKewPTj),0) as BG06_Amaun from BG01_Mohon as a inner join BG01_Mohondt as b on a.BG20_NoMohon = b.BG20_NoMohon where a.BG20_TahunBajet = '{strTahun}' and a.KodKW = '{strKodKW}' and a.KodOperasi = '{strKO}' and a.KodBahagian = '{strKodPTj}' and b.KodVotAm = '{strVotAm}'  and b.BG20_Aktif ='1' and BG20_FlagKB = '1'"
            strSql = $"select isnull(sum(b.BG20_JumKewPTj),0) as BG06_Amaun from BG01_Mohon as a inner join BG01_Mohondt as b on a.BG20_NoMohon = b.BG20_NoMohon where a.BG20_TahunBajet = '{strTahun}' and a.KodKW = '{strKodKW}' and a.KodOperasi = '{strKO}' and a.KodBahagian = '{strKodPTj}' and b.KodVotAm = '{strVotAm}'  and b.BG20_Aktif ='1' and BG20_FlagKB = '1'"
    ...
    Private Function fGetAmtVotAm_K(strTahun, strKO, strKodPTj, strVotAm) As Decimal
        Try

            Dim strSql As String

            'strSql = $"select isnull(sum(b.BG20_JumKewPTj),0) as BG06_Amaun from BG01_Mohon as a inner join BG01_Mohondt as b on a.BG20_NoMohon = b.BG20_NoMohon where a.BG20_TahunBajet = '{strTahun}' and a.KodKW = '{strKodKW}' and a.KodOperasi = '{strKO}' and a.KodBahagian = '{strKodPTj}' and b.KodVotAm = '{strVotAm}'  and b.BG20_Aktif ='1' and BG20_FlagKB = '1'"
            strSql = $"select isnull(sum(b.BG20_JumKewPTj),0) as BG06_Amaun from BG01_Mohon as a inner join BG01_Mohondt as b on a.BG20_NoMohon = b.BG20_NoMohon where a.BG20_TahunBajet = '{strTahun}' and  a.KodOperasi = '{strKO}' and a.KodBahagian = '{strKodPTj}' and b.KodVotAm = '{strVotAm}'  and b.BG20_Aktif ='1' and BG20_FlagKB = '1'"
    ...
    Private Sub fBindGvPTj()
        ...
        Try
            ...
            Dim dbconn As New DBKewConn
            Dim strAmaunMohon As String
            Dim ptjLogint As String

            ds_id = dbconnEQ.fselectCommand(strSql_id)
            ptjLogint = ds_id.Tables(0).Rows(0).Item("att1").ToString


            strSql = $"select distinct dept_kod as dept_kod, dept_name 
                    from live_dept as a
                    where mkod = '{ptjLogint}' and status = '1'
                    order by dept_name"
            Dim ds As New DataSet
            'Dim dbconn As New DBKewConn
            ...
            If Not ds Is Nothing Then
                If ds.Tables(0).Rows.Count > 0 Then
                    ...
                    For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
                        ...
                        If ddlKW.SelectedValue <> "KESELURUHAN" Then 'KESELURUHAN
                        ...
                        Else
                            decAmt10000Op = fGetAmtVotAm_K(strTahun, "01", strKodPTJ, "10000")
                            decAmt10000Ko = fGetAmtVotAm_K(strTahun, "02", strKodPTJ, "10000")
                            decAmt20000Op = fGetAmtVotAm_K(strTahun, "01", strKodPTJ, "20000")
                            decAmt20000Ko = fGetAmtVotAm_K(strTahun, "02", strKodPTJ, "20000")
                            decAmt30000Op = fGetAmtVotAm_K(strTahun, "01", strKodPTJ, "30000")
                            decAmt30000Ko = fGetAmtVotAm_K(strTahun, "02", strKodPTJ, "30000")
                            decAmt40000Op = fGetAmtVotAm_K(strTahun, "01", strKodPTJ, "40000")
                            decAmt40000Ko = fGetAmtVotAm_K(strTahun, "02", strKodPTJ, "40000")
                            decAmt50000Op = fGetAmtVotAm_K(strTahun, "01", strKodPTJ, "50000")
                            decAmt50000Ko = fGetAmtVotAm_K(strTahun, "02", strKodPTJ, "50000")
    ...
    Protected Sub lblCetak_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lblCetak.Click
        ...
        Dim namaLaporan = "BG_PermohonanByPtj&kodkw=" + kodkw + "&kodptj=" + kodptj + "&ptj=" + kodptj
        Dim url = namaLaporan
    ...
    Private Sub fBindGVMohonBajet(KodStatus As String)
        Try
            ...
            Dim strSql_id As String = $"select att1 from live_user_staf where att15='{Session("ssusrID")}'"
            Dim ds_id As New DataSet
            Dim dbconn As New DBKewConn
            Dim strAmaunMohon As String
            Dim ptjLogint As String
            ...
            If KodStatus = String.Empty Then
                strSql = $"select DISTINCT a.BG20_NoMohon, BG20_TarikhMohon, BG20_Program, ISNULL((BG20_AmaunBajet),0) as BG20_AmaunMohon, StatusDok, KodKW, KodOperasi, BG20_Justifikasi from BG01_Mohon as a WHERE a.BG20_Status = 1 and BG20_FlagKB = '1' and KodPtj = '{ptjLogint}' " + ayatSql + " ORDER BY BG20_NoMohon DESC,  BG20_TarikhMohon DESC"
                strSql_count = $"select count(DISTINCT a.BG20_NoMohon) as bil  from BG01_Mohon as a WHERE a.BG20_Status = 1 and BG20_FlagKB = '1'  and KodPtj = '{ptjLogint}' " + ayatSql
            Else
                strSql = $"select DISTINCT a.BG20_NoMohon, BG20_TarikhMohon, BG20_Program, ISNULL((BG20_AmaunBajet),0) as BG20_AmaunMohon, StatusDok, KodKW, KodOperasi, BG20_Justifikasi from BG01_Mohon as a where StatusDok='{KodStatus}' and BG20_FlagKB = '1' and KodPtj = '{ptjLogint}' " + ayatSql + " ORDER BY BG20_NoMohon DESC,  BG20_TarikhMohon DESC"
                strSql_count = $"select count(DISTINCT a.BG20_NoMohon) as bil  from BG01_Mohon as a where StatusDok='{KodStatus}' AND  a.BG20_Status = 1 and BG20_FlagKB = '1'  and KodPtj = '{ptjLogint}' " + ayatSql
            End If
        ...
        ElseIf e.CommandName = "10Op" Or e.CommandName = "10Ko" Or e.CommandName = "20Op" Or e.CommandName = "20Ko" Or e.CommandName = "30Op" Or e.CommandName = "30Ko" Or e.CommandName = "40Op" Or e.CommandName = "40Ko" Or e.CommandName = "50Op" Or e.CommandName = "50Ko" Then
            ...
            Try
                ...
                If lblCheck <> "0.00" Then
                    ...
                    'Open other page.
                    Response.Redirect($"~/FORMS/BAJET/PERMOHONAN BAJET/Maklumat_Agihan_Bajet_Am_KewPTj.aspx?KodSub=" & strKodSub & "&KodSubMenu=" & strKodSubMenu & "&KodPTj=" & strKodPTj & "&Tahun=" & strTahun & "&KodKW=" & strKodKW & "&kodVot=" & strKodVot & "&strKodKO=" & strKodKO & "", False)
    ...
    Protected Sub lbtnHantar_Click(sender As Object, e As EventArgs) Handles lbtnHantar.Click
        ...
        Else
            ...
            If ddlStatus = "1" Then
                statusDok = "19"
                flagKB = " , B20_FlagKew = '1'"
            ElseIf ddlStatus = "0" Then
                statusDok = "20"
                flagKB = " ,  BG20_FlagPemohon = '0', BG20_FlagKB = '0', B20_FlagKew = '0' "
            End If
            ...
            For Each row As GridViewRow In rows
                ...
                If chk.Checked Then
                    ...
                    strSqlButiran = "Update BG01_Mohon set StatusDok = @statusDok , BG20_StaffIDPengesah = @idSemak ,BG20_TarikhSemak = @tarikh,  BG20_UlasanKewPTJ = @ulasan   " + flagKB + "   WHERE BG20_Status = @status and BG20_NoMohon = @NoMohon"
                    paramSqlBtrn = {
            ...
            If countButiran > 0 Then
                fGlobalAlert($"Berjaya simpan", Me.Page, Me.[GetType](), "../../BAJET/PERMOHONAN BAJET/Semakan_Bajet_PTJ.aspx")
            Else
    ...
    Protected Sub lblKemaskiniVot_Click(sender As Object, e As EventArgs) Handles lblKemaskiniVot.Click
        ...
        If dbconn_.fUpdateCommand(strSQL_Save_, paramSql_simpan_) > 0 Then
            ...
            If countButiran > 0 Then
                fGlobalAlert($"Maklumat berjaya dikemaskini.", Me.Page, Me.[GetType](), "../../BAJET/PERMOHONAN BAJET/Semakan_Bajet_PTJ.aspx")
            Else
                fGlobalAlert("Maklumat Butiran tidak berjaya disimpan!", Me.Page, Me.[GetType]())
            End If
        ...
        End If
    End Sub
    Protected Sub lblCetakDt_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lblCetakDt.Click
        ...
        Dim kodkw = ddlKW.SelectedValue
        Dim namaLaporan = "BG_PermohonanDt_PTj&kodkw=" + kodkw + "&kodptj=" + kodptj + "&ptj=" + kodptj
        Dim url = namaLaporan
        Dim strSVR = $"https://reporting.utem.edu.my/ReportServer/Pages/ReportViewer.aspx?%2fS2%2fS2D%2f{url}"
        Dim fullURL As String = $"window.open('{strSVR}', '_blank', 'width=' + (parseInt(window.innerWidth) * 0.3) + ',height=' + (parseInt(window.innerHeight) * .3) + ',status=yes,toolbar=no,menubar=no,location=no,scrollbars=yes,resizable=yes,titlebar=no' );"
        ScriptManager.RegisterStartupScript(Me, GetType(String), "OPEN_WINDOW", fullURL, True)
    ...
    Private Sub gvMohonBajet_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gvMohonBajet.SelectedIndexChanged
        ...
        'Open other page.
        Response.Redirect($"~/FORMS/BAJET/PERMOHONAN BAJET/MaklumatSemakanBajet_KewPtj.aspx?KodSub={KodSub}&KodSubMenu={KodSubMenu}&kodSkrin=1&no={strNoMohon}", False)
    End Sub
End Class
