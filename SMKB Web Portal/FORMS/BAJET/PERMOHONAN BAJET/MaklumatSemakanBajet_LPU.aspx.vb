...
Public Class MaklumatSemakanBajet_LPU
    ...
    Protected Sub gvButiran_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvButiran.RowCommand
        ...
        If e.CommandName = "Padam" Then
            If noMohon <> "" Then
                ...
                Dim strSQL_ = $"UPDATE 
                                BG01_MohonDt SET BG20_JumLPU = @calc, BG20_FlagLPU_Delete = @Status Where BG20_NoMohon = '{noMohon}' and BG20_Id = '{idDt}'"
            ...
            'baca amaun latest
            Dim strSQL2_ = $"SELECT ISNULL(sum(ISNULL(BG20_JumLPU,0)),0) as jumBend FROM BG01_MohonDt Where BG20_NoMohon = '{noMohon}' and BG20_FlagLPU_Delete <> '0' and BG20_Status = '1'"
            ...
            If ds.Tables.Count > 0 Then
                ...
                Dim strSQL_Save = $"UPDATE 
                    BG01_Mohon SET   BG20_AmaunLulusLPU =@amaun, BG20_AmaunBajet = @amaun Where BG20_NoMohon = '{noPermohonan}'"
    ...
    Private Sub MohonEdit(NoMohon As String)
        ...
        Try
            ...
            Dim strSqlDt = $"Select BG20_Id, BG20_NoButiran,BG20_Butiran,BG20_Kuantiti, ISNULL((BG20_JumKetuaPTj),0)  as jumPTj, CASE WHEN BG20_JumNC IS NOT NULL  THEN ISNULL((BG20_JumNC),0) ELSE ISNULL((BG20_JumKetuaPTj),0) END AS BG20_AmaunMohon,BG20_AngJumlah,KodVotSebagai,KodVotAm, BG20_Butiran, isnull(BG20_FlagLPU_Delete,1) as BendDelete from BG01_MohonDt where BG20_NoMohon='{NoMohon}' and BG20_Status = '1'"
    ...
    Protected Sub lbtnKembali_Click(sender As Object, e As EventArgs) Handles lbtnKembali.Click
        ...
        Else
            Response.Redirect("~/FORMS/BAJET/PERMOHONAN BAJET/Kelulusan_LPU.aspx")
        End If
    ...
    Protected Sub lbtnHapus_Click(sender As Object, e As EventArgs) Handles lbtnHapus.Click
        ...
        If countButiran > 0 Then
            fGlobalAlert($"Rekod telah dipadam!", Me.Page, Me.[GetType](), "../../BAJET/PERMOHONAN BAJET/Kelulusan_LPU.aspx")
        Else
                    ...
                    If dbconn.fUpdateCommand(strSQL_Save, paramSql_simpan) > 0 Then
                        ...
                        If countButiran > 0 Then
                            ...
                            Else
                                ...
                                fGlobalAlert($"Rekod baru telah ditambah bersama '{countButiran.ToString}' butiran permohonan!", Me.Page, Me.[GetType](), "../../BAJET/PERMOHONAN BAJET/Kelulusan_LPU.aspx")
                ...
                Else
                    ...
                    Dim paramSql2() As SqlParameter =
                  {
                    ...
                    If dbconn.fInsertCommand(strSql, paramSql) > 0 Then
                        ...
                        If countButiran > 0 Then
                            fGlobalAlert($"Rekod telah dikemaskini bersama '{countButiran.ToString}' butiran permohonan!", Me.Page, Me.[GetType](), "../../BAJET/PERMOHONAN BAJET/Kelulusan_LPU.aspx")
                        Else
                            fGlobalAlert("Maklumat Butiran tidak berjaya disimpan!", Me.Page, Me.[GetType]())
                        End If
