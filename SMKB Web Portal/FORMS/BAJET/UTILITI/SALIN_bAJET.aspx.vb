...
WHERE bg07_IndObjSbg=@IndObjSbg AND (bg07_StatLulus is null or bg07_StatLulus=0)"
    ...
    Private Sub fBindGvPTjAgih()
        ...
        Try
            Dim strSql = "select BG05_IndPTJ, KodPTJ,  (select Butiran from MK_PTJ where MK_PTJ .KodPTJ = bg05_agihptj.KodPTJ) as butPTj, 
            (select isnull(sum(BG20_AmaunMohon),0) as Butiran from BG01_Mohon
            where KodPtj = left(bg05_agihptj.KodPTJ,2) and BG20_TahunBajet = '" & Trim(ddlTahunAgih.SelectedValue.TrimEnd) & "' ) as BG05_Amaun
            from bg05_agihptj where bg05_tahun = '" & Trim(ddlTahunAgih.SelectedValue.TrimEnd) & "'"
    ...
    Private Sub GvPTjAgih_SelectedIndexChanged(sender As Object, e As EventArgs) Handles GvPTjAgih.SelectedIndexChanged
        Try
            ...
            'Open other page.
            Response.Redirect($"~/FORMS/BAJET/UTILITI/Maklumat_Kelulusan_Agihan.aspx?KodSub=" & strKodSub & "&KodSubMenu=" & strKodSubMenu & "&KodPTj=" & strKodPTj & "&Tahun=" & strTahun & "", False)
