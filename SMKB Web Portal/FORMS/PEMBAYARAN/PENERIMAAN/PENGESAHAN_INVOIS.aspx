<%@ Page Title="" Language="vb" AutoEventWireup="false" Async="true" MasterPageFile="~/NestedMasterPage2.master" CodeBehind="PENGESAHAN_INVOIS.aspx.vb" Inherits="SMKB_Web_Portal.PENGESAHAN_INVOIS" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="FormContents" runat="server">
    ...
    <contenttemplate>
            ...
            <div class="modal-dialog modal-dialog-centered modal-xl" style="min-width: 80%" id="modalInvoisDataP" role="document">
                <div class="modal-content">
                        ...
                        <div class="container-fluid mt-3">
                            <ul class="nav nav-tabs" id="subTab" role="tablist">
                                <li class="nav-item" role="presentation" onclick="subTabChange(event)" data-tab="ALL">
                                    <a class="nav-link active" tabindex="-1" data-tab="ALL" aria-disabled="true">Maklumat Ivois</a>
                                </li> 
                                ...
                                <li class="nav-item" role="presentation" onclick="subTabChange(event)" data-tab="LJR">
                    ...
                    <div class="modal-body">
                           <div id="INV" class="modal-sub-tab mt-2">
                                ...
                                <div class="col-md-12">
                                    <div class="row justify-content-center">
                                        ...
                                        <div class="col-md-3">
                                            ...
                                            <div class="form-group input-group">
                                                <input type="text"  style="cursor: help;"  class=" input-group__input form-control input-sm "  onclick="showPenerima(this)" placeholder=""  id="pnrmInv" readonly>
                                                <label class="input-group__label">Penerima Invois</label>
                                                <i  id="pnrmIco" class=" fa fa-info-circle" aria-hidden="true"></i>
                                            </div>
                            ...
                            <div class="row justify-content-center">
                                ...
                                <div class="col-md-3">
                                            ...
                                            <div class="form-group input-group">
                                                <input type="text" class=" input-group__input form-control input-sm " placeholder="" name="tkhDO" id="jenInv" readonly>
                        ...
                        <div id="LJR" class="modal-sub-tab mt-2">

                            <div class="row">
                                <div class="col-md-4">

                                    <div class="input-group form-group">
                                        <select class=" input-group__input  form-control input-sm ui search dropdown" id="ddlPilihanVot" onchange="pilihVot(event)">
                                        </select>
                                        <label class=" input-group__label">Vot Pemiutang</label>
                                    </div>
                                </div>
                                <div class="col-md-12">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 ">
                                    <div class="row justify-content-end">
                                        ...
                                        <button type="button" class="btn btn-danger btnXLulus" data-toggle="tooltip" title="Simpan dan Hantar">Tidak Lulus</button>
                                        <p>&nbsp;</p>  
                                        <button type="button" onclick="lulus()" id="btnLulus" class="btn btn-success btnLulus" data-toggle="tooltip" title="Simpan dan Hantar">Lulus</button>
        ...
        <script type="text/javascript">
            ...
            var cachepilihanVot = []
            var selectedVot = null

            //pair of kod kump w, kod op, kod proj and sum val
            var cacheKodPair = []

            async function lulus() {


                let conf = await show_message_async('Lulus invois?<br>  <textarea id="txtUlasan" placeholder="Ulasan" class="form-control" rows = "1" ></textarea > ')
                if (!conf) {
                    return
                }
                let ulasan = $("#txtUlasan").val()  

                let data = tblLejar.rows().data()
                console.log(data)
                ...
                fulldata.Kod_Vot = $("#ddlPilihanVot").val()
                fulldata.details = dtlrow


                console.log(fulldata)
                let res = await lulusInvois(fulldata, ulasan).catch(function (err) {
                    console.log(err)
                })
                if (!res) {
                    notification("Gagal diluluskan")
                    console.log(res.Message)
                    return
                }
                notification("Invois berjaya diluluskan")
            ...
            function lulusInvois(invois, ulasan) {
                return new Promise(function (resolve, reject) {
                    let url = '<%= ResolveUrl("~/FORMS/PEMBAYARAN/Invois_WS.asmx/lulusInvois") %>'
                    $.ajax({
                        url: url,
                        method: "POST",
                        data: JSON.stringify({
                    ...
                    $.ajax({
                        ...
                        success: async function (data) {
                            ...
                            if (data.dtl) {
                                ...
                                let votload = await getPilihanKodVot(data.hdr[0].Tarikh_Dibuat).catch(function (err) { //assume tarikh dibuat
                                    console.log(err)
                                })
                                if (!votload) {
                                    dialogMakluman("masalah dalam mendapatkan data pilihan vot dari sistem")
                                }
                                if (cachepilihanVot) {
                                    cachepilihanVot.forEach(v => {
                                        arrvot.push(v.Kod_Vot)
                                    })
                                }
                                ...
                                $("#pnrmInv").val(getPemiutang(data.dtl[0].Kod_Pemiutang).Nama_Pemiutang)   
                                document.getElementById("pnrmInv").dataset.kp = data.dtl[0].Kod_Pemiutang 
            ...
            function loadInvoisLists(dateStart, dateEnd) {
                ...
                show_loader()
                $.ajax({
                    url: '<%= ResolveUrl("~/FORMS/PEMBAYARAN/Invois_WS.asmx/getInvoisHantar") %>',
                    method: "POST",
                    data: JSON.stringify({
                        DateStart: dateStart,
                        DateEnd: dateEnd
            ...
            function pilihVot(e) {
                ...
                if (selectedVot != newval) {
                    //do changes 
                    tblLejar.rows().every(function (rowIdx, tableLoop, rowLoop) {
                        let dt = tblLejar.row(this).data();
                        console.log(dt)
                        if (dt.Kod_Vot == selectedVot) {

                            dt.Kod_Vot = newval
                            tblLejar.row(this).invalidate();
                        }
                        //  let data = this.data()
                        //  if (data.Kod_Vot == selectedVot) { 
                        //      this.data("Kod_Vot", newval) 
                        //   } 
