<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/NestedMasterPage2.master" CodeBehind="PENYEDIAAN_BAUCAR.aspx.vb" Inherits="SMKB_Web_Portal.PENYEDIAAN_BAUCAR" %>
    ...
    <contenttemplate>
                    ...
                    <div class="modal-content">
                                ...
                                <div class="row mt-3">
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <select class="input-group__input form-control ui search dropdown" id="ddlBank">
                                            </select>
                                    ...
                                    <div class="col-md-6">
                                        <div class="input-group">
                                            <textarea id="txtButiranBaucar" class="input-group__input form-control" name="message" rows="1"></textarea>
                        ...
                        <div class="modal-footer">
                            <div class="form-group col-md-12" align="right">
                                <button type="button" onclick="cancelBaucar()" class="btn btn-danger btnXLulus" data-toggle="tooltip" data-placement="bottom" title="Batal">Batal</button>
                                <button type="button" onclick="saveBaucar()" class="btn btn-secondary" data-toggle="tooltip" data-placement="bottom" title="Simpan">Simpan</button>
                                <button type="button" onclick="saveBaucar(true)" class="btn btn-success btnLulus" data-toggle="tooltip" data-placement="bottom" title="Lulus">Hantar</button>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <script type="text/javascript">
            ...
            async function lulusBaucar() {
                ...
                // baucar hdr
                var baucar = cacheBaucar
                if (!baucar.Kod_Bank) {
                    baucar.Kod_Bank = $("#ddlBank").dropdown('get value');
                }



                tblPenerima.rows().data().each(function (rowdata) {
                    rowdata.Jumlah_Bayar = formattedPriceToFloat(rowdata.Jumlah_Bayar)
                })

                var dataPenerima = tblPenerima.data().toArray()
                baucar.details = dataPenerima
                // prepare data end
                console.log(baucar)
                show_loader()
                console.log('invois hdr')
                console.log(cacheInvoisHdr)
                var hdrStatus = await postLulusBaucar(baucar, cacheInvoisHdr.Kod_Vot).catch(function (err) {
            ...
            function postLulusBaucar(baucar, votInvois) {
                return new Promise(function (resolve, reject) {
                    $.ajax({
                        url: '<%= ResolveUrl("~/FORMS/PEMBAYARAN/Baucar_WS.asmx/lulusBaucar") %>',
                        method: "POST",
                        data: JSON.stringify({
                            "baucar": baucar,
                            "votInvois": votInvois
            ...
            $(document).ready(function () {
                ...
                console.log("test")
                $.ajax({
                    url: '<%= ResolveUrl("~/FORMS/PEMBAYARAN/Baucar_WS.asmx/ping") %>',
                    method: "GET",
                    data: JSON.stringify(""),
                    dataType: "json",
                    contentType: "application/json;charset=utf-8",
                ...
                $.ajax({
                    url: '<%= ResolveUrl("~/FORMS/PEMBAYARAN/Invois_WS.asmx/ping") %>',
                    method: "GET",
                    data: JSON.stringify(""),
                    dataType: "json",
                    contentType: "application/json;charset=utf-8",
                ...
                tblPenerima = $("#tblPenerimaBaucar").on('draw.dt', function () {
                    //show nothing
                    if (document.getElementsByClassName("ddlCaraBayar").length > 0) {
                        $(".ddlCaraBayar").dropdown({
                            ...
                            onChange: function (value, text, $selectedItem) {
                                ...
                                var tdata = tblPenerima.row(curTR).data()
                                tdata.Cara_Bayar = value
                            },
                            apiSettings: {
                                url: '<%= ResolveUrl("~/FORMS/PEMBAYARAN/Baucar_WS.asmx/getSenaraiCaraBayar") %>' + '?q={query}',
                                method: 'POST',
                                dataType: "json",
                                contentType: 'application/json; charset=utf-8',
                                cache: false,
                                ...
                                onSuccess: function (response, element, xhr) {
                                    ...
                                    if (res.Code == "200") {
                                        // good request
                                        var listOptions = res.Payload
                                        $.each(listOptions, function (index, option) {
                                            $(objItem).append($('<div class="item" data-value="' + option.Kod_Detail + '">').html(option.Kod_Detail + ' - ' + option.Butiran));
                                        });
                                    }
                                    ...
                                    let preselected = $(element).find('select').data('selected')
                                    $(obj).dropdown('set selected', preselected);
                        ...
                        $(".ddlCaraBayar").dropdown('show')
                        $(".ddlCaraBayar").dropdown('hide')

                        $('.ddlCaraBayar').each(function (index, element) {
                            console.log(element);
                            // You can log any other details or manipulate the elements as needed
                        });
                    }
                    if (document.getElementsByClassName("ddlKodCukai").length > 0) {

                        $(".ddlKodCukai").dropdown({
                            selectOnKeydown: true,
                            fullTextSearch: true,
                            onChange: function (value, text, $selectedItem) {
                                ...
                                var curTR = $(this).closest("tr");
                                var tdata = tblPenerima.row(curTR).data()
                                tdata.Kod_Cukai = value
                            },
                            direction: 'down',
                            placeholder: '- Sila Pilih -',
                            apiSettings: {

                                url: '<%= ResolveUrl("~/FORMS/PEMBAYARAN/Baucar_WS.asmx/getSenaraiKodCukai") %>' + '?q={query}',
                                method: 'POST',
                                dataType: "json",
                                contentType: 'application/json; charset=utf-8',
                                cache: false,
                                ...
                                onSuccess: function (response, element, xhr) {
                                    ...
                                    // Refresh dropdown
                                    $(obj).dropdown('refresh');
                                    $(obj).dropdown('set selected', $(element).find('select').data('selected'));
                                    let pop = $(obj).find("input").is(":focus")
                                    if (pop) {
                                        $(obj).dropdown('show');
                                    }
                        ...
                        $(".ddlKodCukai").dropdown('show')
                        $(".ddlKodCukai").dropdown('hide')
                    }
                }).DataTable({
                    "responsive": true,
                    "paging": false,
                    ...
                    columns: [
                        ...
                        },
                        {
                            data: 'Jumlah_Bayar',
                            render: function (data, type, row) {
                                if (row.Jumlah_Perlu_Bayar) {
                                    return '<input class="form-control inputBayar text-right"  name="Jumlah_Bayar" min="0" max="' + row.Jumlah_Perlu_Bayar
                                        + '" type="text" value="' + formatPrice(data) + '"><span>'
                                        + 'Perlu Bayar: ' + formatPrice(row.Jumlah_Perlu_Bayar) + '</span>'
                                }
                                else {
                                    return '<input class="form-control inputBayar" name="Jumlah_Bayar" min="0" type="number" value="' + data + '">'
                                }
                        ...
                        },
                        {
                            ...
                            render: function (data, type, row) {
                                ...
                                var html = '<div class="input-group" ><select data-selected="' + data + '" class="form-control ui search dropdown ddlKodCukai">'
                                    + '</select></div > '
                        ...
                        },
                        {
                            ...
                            render: function (data, type, row) {
                                // Generate dropdown HTML
                                var html = '<div class="input-group" ><select data-selected="' + data + '" class="form-control ui search dropdown ddlCaraBayar">'
                                    + '</select></div > '
                ...
                $('#ddlBank').dropdown({
                    ...
                    placeholder: '- Sila Pilih -',
                    onChange: function (value, text, $selectedItem) {
                        console.log('ddlbank', value)
                        changeLejarVotBank(value)
                    },
                    apiSettings: {
                        url: '<%= ResolveUrl("~/FORMS/PEMBAYARAN/Baucar_WS.asmx/getSenaraiBank") %>' + '?q={query}',
                        method: 'POST',
                        dataType: "json",
                        contentType: 'application/json; charset=utf-8',
                        cache: false,
                        ...
                        onSuccess: function (response) {
                            ...
                            if (res.Code == "200") {
                                ...
                                var listOptions = res.Payload
                                $.each(listOptions, function (index, option) {
                                    $(objItem).append($('<div class="item" data-value="' + option.Kod_Bank + '">').html(option.Kod_Bank + ' - ' + option.Nama_Bank));
                                });
                            }
                ...
                $('#ddlBank').dropdown('show');
                $('#ddlBank').dropdown('hide');
            ...
            function loadInvoisLists(dateStart, dateEnd) {
                ...
                show_loader()
                $.ajax({
                    url: '<%= ResolveUrl("~/FORMS/PEMBAYARAN/Baucar_WS.asmx/getInvoisDrafBaucar") %>',
                    method: "POST",
                    data: JSON.stringify({
                        DateStart: dateStart,
                        DateEnd: dateEnd
      ...
      <%--          "ajax": {
            ...
            function loadBaucarIntoModal(baucar) {
                ...
                if (baucar.Kod_Bank) {
                    $("#ddlBank").dropdown('set selected', baucar.Kod_Bank);
                }
                else {
                ...
                return new Promise(function (resolve, reject) {
                    ...
                    $.ajax({
                        ...
                        success: async function (data) {
                            ...
                            let votbank = $("#ddlBank").dropdown('get value');
            ...
            async function saveBaucar(hantar = false) {
                // confirmation
                let save = await dialogPengesahan("Simpan Maklumat Baucar?")
                if (!save) {
                    return
                }
                var errors = []
                ...
                //readonly value getback rom cache
                baucar['No_Baucar'] = cacheBaucar['No_Baucar'];
                baucar['Tarikh'] = cacheBaucar['Tarikh'];
                baucar['Butiran'] = $("#txtButiranBaucar").val();

                // value from input
                baucar['Kod_Bank'] = $("#ddlBank").dropdown('get value');
                if (baucar.Kod_Bank == "") {
                    errors.push("Sila Pilih Kod Bank")
                }
                // baucar penerima
                ...
                if (!hantar) {
                    ...
                    hdrStatus = await saveBaucarHdr(baucar).catch(function (err) {
                        dialogMakluman("Baucar gagal disimpan: " + err)
                    })
                }
                else {
                    hdrStatus = await submitBaucar(baucar).catch(function (err) {
                        dialogMakluman("Baucar gagal disimpan: " + err)
                    })
                }
                close_loader()
                if (hdrStatus) {
                    dialogMakluman("Baucar Berjaya Disimpan")
                    if (hantar) {
            ...
            function submitBaucar(baucar) {
                return new Promise(function (resolve, reject) {
                    $.ajax({
                        url: '<%= ResolveUrl("~/FORMS/PEMBAYARAN/Baucar_WS.asmx/SubmitBaucar") %>',
                        method: "POST",
                        data: JSON.stringify({
                            "baucar": baucar
                        }),
            ...
            function saveBaucarHdr(baucar) {
                return new Promise(function (resolve, reject) {
                    $.ajax({
                        url: '<%= ResolveUrl("~/FORMS/PEMBAYARAN/Baucar_WS.asmx/SaveBaucar") %>',
                        method: "POST",
                        data: JSON.stringify({
                            "baucar": baucar
                        }),
            ...
            async function cancelBaucar() {
                let confirm = await dialogPengesahan("Batal baucar?")
                if (!confirm)
                    return
                ...
                var baucar = {}
                baucar['No_Baucar'] = cacheBaucar['No_Baucar'];
                baucar['Status_Dok'] = "31"
                console.log('sini')
                let status = await saveBaucarHdr(baucar).catch(function (err) {
                    dialogMakluman("Proses pembatalan tidak berjaya: " + err)
                })
                if (status) {
                    let batalInvois = await dialogPengesahan("Baucar telah dibatalkan<br>Adakah anda ingin membatalkan invois juga?")
                    if (!batalInvois)
                        return

                    var invois = {}
                    invois["ID_Rujukan"] = cacheBaucar['ID_Rujukan'];
                    invois["Status_Dok"] = "20"

                    let invstatus = await updateInvois(invois).catch(function (err) {
                        dialogMakluman("Proses pembatalan invois tidak berjaya<b>" + err)
                    })

                    if (invstatus) {
                        dialogMakluman("Invois berjaya dibatalkan")
                        $("#btnApplyInvoisDateFilter").click()
                    }
            ...
            function updateInvois(invois) {

                return new Promise(function (resolve, reject) {
                    $.ajax({
                        url: '<%= ResolveUrl("~/FORMS/PEMBAYARAN/Invois_WS.asmx/updateInvoisHdr") %>',
                        method: "POST",
                        data: JSON.stringify({
                            "invois": invois
                        }),
